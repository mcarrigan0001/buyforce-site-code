...
(function () {
  ...
  function bfPipeCSS(){ if(window.__bfPipeCSS) return; window.__bfPipeCSS=1; var s=document.createElement('style'); s.textContent="#bff-wrap{font-family:Inter,system-ui,sans-serif;background:#0d0d0f;border-radius:16px;padding:22px 22px 18px;color:#e8e8ea;box-shadow:0 6px 24px rgba(0,0,0,.25);}#bff-wrap *{box-sizing:border-box;}.bff-head{display:flex;align-items:center;gap:14px;margin-bottom:18px;flex-wrap:wrap;}.bff-title{font-size:18px;font-weight:700;color:#fff;letter-spacing:-.3px;}.bff-tabs{display:flex;gap:6px;margin-left:auto;background:#1b1b1f;border-radius:10px;padding:3px;}.bff-tab{font-size:12px;font-weight:600;color:#9a9aa2;padding:6px 14px;border-radius:8px;cursor:pointer;border:none;background:none;}.bff-tab.on{background:#2b2b31;color:#fff;}.bff-scope{font-size:11px;color:#7a7a82;font-weight:500;}.bff-funnel{display:flex;align-items:flex-start;flex-wrap:wrap;gap:4px 2px;}.bff-stage{display:flex;flex-direction:column;align-items:center;min-width:92px;padding:8px 6px;border-radius:12px;cursor:pointer;}.bff-stage:hover,.bff-stage.on{background:#17171b;}.bff-num{font-size:30px;font-weight:800;color:#1fc77d;line-height:1;}.bff-num.zero{color:#54545c;}.bff-lab{font-size:11px;color:#b6b6be;margin-top:7px;text-align:center;line-height:1.25;max-width:104px;}.bff-chev{align-self:center;color:#3a3a42;font-size:22px;font-weight:800;padding:0 2px;margin-top:10px;}.bff-list{margin-top:16px;border-top:1px solid #232329;padding-top:14px;display:none;}.bff-list.show{display:block;}.bff-list-h{display:flex;align-items:center;gap:8px;margin-bottom:6px;}.bff-list-t{font-size:13px;font-weight:700;color:#fff;}.bff-list-c{font-size:12px;color:#8a8a92;}.bff-colh{display:flex;align-items:center;gap:12px;padding:6px 10px;font-size:10px;font-weight:700;letter-spacing:.5px;color:#6a6a72;text-transform:uppercase;}.bff-row{display:flex;align-items:center;gap:12px;padding:9px 10px;border-radius:9px;text-decoration:none;color:#dcdce2;cursor:pointer;border-top:1px solid #1c1c21;}.bff-row:hover{background:#17171b;}.bff-veh{font-size:13px;font-weight:600;color:#f1f1f3;flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}.bff-sell{font-size:12px;color:#9a9aa2;flex:none;width:130px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}.bff-days{font-size:12px;flex:none;width:74px;text-align:right;font-weight:600;}.bff-act{font-size:12px;color:#9a9aa2;flex:none;width:84px;text-align:right;}.bff-open{flex:none;width:54px;text-align:right;color:#1fc77d;font-size:12px;font-weight:600;}.bff-empty,.bff-load{font-size:13px;color:#8a8a92;padding:10px 2px;}@media(max-width:640px){.bff-chev{display:none;}.bff-stage{min-width:31%;}.bff-sell,.bff-act{display:none;}}"; document.head.appendChild(s); }
  function bfPipeTitle(){ try{ var els=document.querySelectorAll('h1,h2,h3,[class*="itle"]'); for(var i=0;i<els.length;i++){ var e=els[i]; if(e.getAttribute('data-bfpipetitle')==='1') continue; if(e.children.length<=1 && /^pipeline summary$/i.test((e.textContent||'').trim())){ e.style.setProperty('color','#ffffff','important'); e.style.setProperty('text-shadow','0 0 8px rgba(31,199,125,.9), 0 0 18px rgba(31,199,125,.55)','important'); e.setAttribute('data-bfpipetitle','1'); } } }catch(e){} }
  function bfPipeRender(R){
    ...
    var MACRO=[{label:'New',statuses:['FRESH_LEADS','ENGAGED_AWAITING_VIN']},{label:'Appraising',statuses:['VIN_RECEIVED_APPRAISAL_NEEDED','APPRAISAL_COMPLETE_ENTER_OFFER_SHEET_VALUES','APPRAISAL_REVIEW_NEEDED','APPRAISAL_REVIEW_COMPLETE']},{label:'Offer Out',statuses:['OFFER_SHEET_GENERATED','OFFER_SENT_0_2_DAYS','NURTURING_FOLLOW_UP_AND_RE_ENGAGE']},{label:'Closing',statuses:['VERBAL_YES_SCHEDULE_APPT','SCHEDULED','APPT_SHOWN_FOLLOW_UP']},{label:'Acquired',statuses:['ACQUIRED']},{label:'No Deal',statuses:['NO_DEAL']}];
    var st={view:'stages',data:null,open:null};
    R.innerHTML='<div id="bff-wrap"><div class="bff-head"><span class="bff-title">Sales Pipeline</span><span class="bff-scope" data-r="scope"></span><div class="bff-tabs"><button class="bff-tab on" data-view="stages">Stages</button><button class="bff-tab" data-view="macro">Macro</button></div></div><div class="bff-funnel" data-r="funnel"><div class="bff-load">Loading pipeline…</div></div><div class="bff-list" data-r="list"></div></div>';
    function q(n){ return R.querySelector('[data-r="'+n+'"]'); }
    function esc2(s){ return (s==null?'':String(s)).replace(/[&<>"]/g,function(c){return{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c];}); }
    function daysVal(r){ if(r.daysInStage!=null) return r.daysInStage; var t=new Date(r.stageEnteredAt||0).getTime(); if(isNaN(t)) return 0; return Math.floor((Date.now()-t)/86400000); }
    function daysTxt(r){ var d=daysVal(r); return d<=0?'today':(d+'d'); }
    function daysColor(d){ if(d>=14) return '#ff5b5b'; if(d>=7) return '#f0a23a'; if(d>=4) return '#e8d44a'; return '#7d7d85'; }
    function ago(iso){ if(!iso) return '—'; var t=new Date(iso).getTime(); if(isNaN(t)) return '—'; var s=Math.floor((Date.now()-t)/1000); if(s<60) return 'just now'; var m=Math.floor(s/60); if(m<60) return m+'m ago'; var h=Math.floor(m/60); if(h<24) return h+'h ago'; var d=Math.floor(h/24); if(d<30) return d+'d ago'; return new Date(t).toLocaleDateString(); }
    ...
    function renderList(){ var L=q('list'); if(!st.open){ L.className='bff-list'; L.innerHTML=''; return; } var u=units().filter(function(x){return x.status===st.open;})[0]; if(!u){ L.className='bff-list'; L.innerHTML=''; return; } var rows=(u.records||[]).slice().sort(function(a,b){return daysVal(b)-daysVal(a);}); var head=rows.length?'<div class="bff-colh"><span class="bff-veh">Vehicle</span><span class="bff-sell">Seller</span><span class="bff-days">In stage</span><span class="bff-act">Last activity</span><span class="bff-open"></span></div>':''; var body=rows.length?rows.map(function(r){ var d=daysVal(r); return '<a class="bff-row" href="'+RECORD_BASE+esc2(r.uuid)+'"><span class="bff-veh">'+(esc2(r.title)||'(untitled)')+'</span><span class="bff-sell">'+esc2(r.seller||'')+'</span><span class="bff-days" style="color:'+daysColor(d)+';">'+daysTxt(r)+'</span><span class="bff-act">'+esc2(ago(r.lastActivityAt))+'</span><span class="bff-open">Open →</span></a>'; }).join(''):'<div class="bff-empty">No deals in this stage.</div>'; L.className='bff-list show'; L.innerHTML='<div class="bff-list-h"><span class="bff-list-t">'+esc2(u.label)+'</span><span class="bff-list-c">'+u.count+' deal'+(u.count===1?'':'s')+' · sorted by most days in stage</span></div>'+head+body; }
    function render(){ var F=q('funnel'); var U=units(); if(!U.length){ F.innerHTML='<div class="bff-empty">No data.</div>'; return; } var h=''; U.forEach(function(u,i){ if(i>0) h+='<div class="bff-chev">»</div>'; var on=(st.open===u.status)?' on':''; var z=u.count===0?' zero':''; h+='<div class="bff-stage'+on+'" data-status="'+esc2(u.status)+'"><div class="bff-num'+z+'">'+u.count+'</div><div class="bff-lab">'+esc2(u.label)+'</div></div>'; }); F.innerHTML=h; [].forEach.call(F.querySelectorAll('.bff-stage'),function(el){ el.addEventListener('click',function(){ var s2=el.getAttribute('data-status'); st.open=(st.open===s2?null:s2); render(); renderList(); }); }); renderList(); }
    [].forEach.call(R.querySelectorAll('.bff-tab'),function(t){ t.addEventListener('click',function(){ R.querySelectorAll('.bff-tab').forEach(function(x){x.classList.remove('on');}); t.classList.add('on'); st.view=t.getAttribute('data-view'); st.open=null; render(); }); });
    var tk=''; try{ tk=bfGetApiToken(); }catch(e){}
    fetch(ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json','X-BF-Token':tk},body:'{}'}).then(function(r){return r.json();}).then(function(d){ st.data=d||{stages:[]}; var sc=q('scope'); if(d&&d.needsAuth){ sc.textContent='(open a record once so your access loads, then refresh)'; } else { sc.textContent=(d&&d.allAccess)?('All dealerships · '+(d.total||0)+' deals'):('Your dealership · '+(d.total||0)+' deals'); } render(); }).catch(function(){ q('funnel').innerHTML='<div class="bff-empty">Could not load the pipeline. Refresh to retry.</div>'; });
  }
  function bfPipelinePage(){ try{ var marker=null; var tw=document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null); var tn; while(tn=tw.nextNode()){ if((tn.nodeValue||'').indexOf('BF_PIPELINE_FUNNEL')>-1){ marker=tn.parentElement; break; } } if(!marker) return; bfPipeTitle(); if(marker.getAttribute('data-bfpipe')==='1') return; marker.setAttribute('data-bfpipe','1'); bfPipeCSS(); bfPipeRender(marker); }catch(e){} }
