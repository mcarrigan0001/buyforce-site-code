...
(function () {
  ...
  var BF_CUSTOM_MILESTONES=(function(){try{return localStorage.getItem('bfCustomMs')!=='0';}catch(e){return true;}})();  // Phase 1 ON for all; kill-switch: localStorage bfCustomMs=0 restores the iframe
  ...
  function money(s){ return (s==null?'':String(s)).replace(/\.00(?=$|[^0-9])/,''); }
  ...
  function bfLitFunnelForRecord(){
    ...
    function lblOf(s){ return ((s.querySelector('.bff-l')||{}).textContent||'').trim(); }
    if(location.pathname.indexOf('/preview/')>-1){
      ...
      if(!wrap.hasAttribute('data-dashstage')){ var cs=''; [].forEach.call(stages,function(s){ if(s.classList.contains('on')) cs=lblOf(s); }); wrap.setAttribute('data-dashstage', cs); }
      wrap.setAttribute('data-recstage', macro);
      [].forEach.call(stages,function(stg){ stg.classList.toggle('on', lblOf(stg)===macro); });
    } else if(wrap.hasAttribute('data-recstage')){
      var rec=wrap.getAttribute('data-recstage'); var dash=wrap.getAttribute('data-dashstage')||'';
      wrap.removeAttribute('data-recstage'); wrap.removeAttribute('data-dashstage');
      if(rec && rec!==dash){ var target=null; [].forEach.call(stages,function(stg){ if(lblOf(stg)===rec) target=stg; }); if(target) try{ target.click(); }catch(e){} }
    }
  }
  ...
  function bfUserInfoRole(){
    ...
    var slotEl=null;
    txt.forEach(function(e){ if(e.getAttribute('data-bfrole')==='1'){ slotEl=e; } else if(/@/.test(e.textContent||'')){ slotEl=slotEl||e; } });
    if(!slotEl) return;
    bfFetchRole();
  ...
  function bfNavRec(u){ if(!u) return; var _url='https://buyforce.noloco.co/command/preview/'+u+'/overview'; try{ location.assign(_url); }catch(e){ location.href=_url; } }
  function bfFlipGo(u){ bfNavRec(u); }
  ...
  function bfRecMobNav(){
    ...
    var _mm=location.pathname.match(/\/(rec[0-9a-z]+)/i); var _uu2=_mm?_mm[1]:''; var _ik=/^\/pipeline\//.test(location.pathname); var order=bfFlipOrder; if(!order||!order.some(function(o){return o.u===_uu2;})||(order.__src!==(_ik?'board':'pipe'))){ order=_ik?bfBuildFullOrder():(bfPipeFlipOrder(_uu2)||bfBuildFullOrder()); if(order){ order.__src=_ik?'board':'pipe'; bfFlipOrder=order; } }
  ...
  function bfRecFlip(){
    ...
    var _isKan=/^\/pipeline\//.test(location.pathname);
    var order=bfFlipOrder;
    var stale=!order||!order.some(function(o){return o.u===uuid;})||(order.__src!==(_isKan?'board':'pipe'));
    if(stale){ order=_isKan?bfBuildFullOrder():(bfPipeFlipOrder(uuid)||bfBuildFullOrder()); if(order){ order.__src=_isKan?'board':'pipe'; bfFlipOrder=order; bfFlipCount=cnt; } }
    if(!order||!order.length) return;
  ...
  function bfPipeGo(u){ bfNavRec(u); }
  ...
  function bfV2Html(R,uuid){
    ...
    var econ='<div class="bfc-econrow">'
      +'<div class="bfc-offstrip">'
        ...
        +'<div class="bfc-offcell"><div class="bfc-offlbl">Spread</div><div class="bfc-offval" data-bfspread>'+(spread!=null?M(spread):'\u2014')+'</div><div class="bfc-offsub lime" data-bfspct'+(spct!=null?'':' hidden')+'><i class="ti ti-chevron-up" aria-hidden="true"></i>'+(spct!=null?spct:0)+'% difference</div></div>'
      +'</div>'
  ...
  function bfV2Wire(host, uuid, R){
    ...
    var pcb=host.querySelector('[data-bfpaidoff]');
    if(pcb&&pin){
  ...
  // (kanban cards now use Noloco's native record open)
  ...
  document.addEventListener('click', function(e){
    ...
    try{ history.back(); }catch(_){ location.href='/command'; }
  }, true);
  var bfEditing=false;
  document.addEventListener('click', function(e){
  ...
  function bfPostTask(p){ if(BF_TESTMODE && (!p||p.action==='list')) return Promise.resolve({tasks:[]}); try{ if(p&&p.action==='list'&&p.uuid){ var c=bfTaskCache[p.uuid]; if(c && (Date.now()-c.at)<BF_WSTTL) return Promise.resolve(c.data); } if(p&&(p.action==='create'||p.action==='complete')){ if(p.uuid) delete bfTaskCache[p.uuid]; else bfTaskCache={}; } }catch(_c){} return fetch(BF_WH+'/tasks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(p)}).then(function(r){return r.json();}).then(function(d){ try{ if(p&&p.action==='list'&&p.uuid) bfTaskCache[p.uuid]={at:Date.now(),data:d}; }catch(_c2){} return d; }).catch(function(){return null;}); }
  ...
  function bfPipeRender(R){ try{ R.style.visibility='visible'; if(window.__bfStopPrehide) window.__bfStopPrehide(); }catch(_e){}
    ...
    var RECORD_BASE='https://buyforce.noloco.co/command/preview/';
    ...
    function fOpp(r){ var f=st.f; if(st.repMaster){ if((r.rep||'').trim()!==st.repMaster) return false; } if(f.mstage){ if(macroLabelOf(r.status)!==f.mstage) return false; } if(f.stage){ if((r.status||'')!==f.stage) return false; } if(f.comp){ if((r.competition||'').toLowerCase().indexOf(f.comp)<0) return false; } if(f.rep){ if(f.rep==='__none__'){ if(r.rep) return false; } else if(r.rep!==f.rep) return false; } if(f.ostatus){ if((r.offerStatus||'')!==f.ostatus) return false; } if(!nIn(r.daysInStage,f.daysMin,f.daysMax)) return false; if(!nIn(r.mileage,f.mileMin,f.mileMax)) return false; if(!nIn(r._score,f.scoreMin,f.scoreMax)) return false; if(!nIn(r._ovp,f.ovpMin,f.ovpMax)) return false; if((f.driveMin!==''&&f.driveMin!=null)||(f.driveMax!==''&&f.driveMax!=null)){ var dm=dMin(r.driveTime); if(dm<0) return false; if(!nIn(dm,f.driveMin,f.driveMax)) return false; } if(!nIn(bfVehField(r,'year'),f.yearMin,f.yearMax)) return false; var vsel=[['make','make'],['model','model'],['trim','trim'],['bodyStyle','bodyStyle'],['drivetrain','drivetrain'],['engine','engine'],['fuelType','fuelType'],['transmission','transmission'],['cabType','cabType']]; for(var _vi=0;_vi<vsel.length;_vi++){ var _fk=vsel[_vi][0]; if(f[_fk]){ if(String(bfVehField(r,vsel[_vi][1])||'')!==f[_fk]) return false; } } return true; }
    ...
    function ovlShow(){ var uuid=ovlList[ovlIdx]; if(!uuid||!ovl) return; ovlIf.src=RECORD_BASE+uuid+'/overview'; (function(){ var ld=ovl.querySelector('[data-o="load"]'); if(!ld) return; ld.style.display='flex'; var n=0; clearInterval(ovl.__ldT); ovl.__ldT=setInterval(function(){ n++; var rdy=false; try{ var d=ovlIf.contentDocument; rdy=!!(d&&d.documentElement&&d.documentElement.getAttribute('data-bfv2')==='1'); }catch(e){} if(rdy||n>30){ clearInterval(ovl.__ldT); ld.style.display='none'; } }, 100); })(); var op=ovl.querySelector('[data-o="open"]'); if(op) op.href=RECORD_BASE+uuid+'/overview'; var ps=ovl.querySelector('[data-o="pos"]'); if(ps) ps.textContent=(ovlIdx+1)+' of '+ovlList.length; var pv=ovl.querySelector('[data-o="prev"]'); var nx=ovl.querySelector('[data-o="next"]'); if(pv) pv.style.visibility=ovlIdx>0?'visible':'hidden'; if(nx) nx.style.visibility=ovlIdx<ovlList.length-1?'visible':'hidden'; }
    ...
    function bfWsPrefetch(){
      if(BF_TESTMODE) return;  // no neighbor prefetch while conserving n8n executions
    ...
    function bfModalRender(){
      ...
      var R={}; try{ R=JSON.parse(sessionStorage.getItem('bfrec:'+uuid)||localStorage.getItem('bfrec:'+uuid)||'{}'); }catch(e){}
      if(!R||Object.keys(R).length<3){ bfNavRec(uuid); return; }
    ...
    function bfOpenModal(uuid){
      if(!uuid) return;
      bfModalList=navList(); bfModalIdx=bfModalList.indexOf(uuid);
      if(bfModalIdx<0){ bfModalList=[uuid]; bfModalIdx=0; }
      try{ sessionStorage.setItem('bf.pipeorder', JSON.stringify(bfModalList)); }catch(e){}
      var m=bfEnsureModal(); m.style.display='block'; document.documentElement.setAttribute('data-bfmodal','1');
      bfModalRender();
    }
    function openOverlay(uuid){ if(!uuid) return; bfOpenModal(uuid); }
    function lcTfRange(key){ var now=new Date(),y=now.getFullYear(),m=now.getMonth(),nowT=now.getTime(),MS=864e5; function D(yy,mm,dd){return new Date(yy,mm,dd,0,0,0,0).getTime();} function sod(t){var x=new Date(t);return new Date(x.getFullYear(),x.getMonth(),x.getDate()).getTime();} if(key==='today')return{start:sod(nowT),end:nowT}; if(key==='thisweek'){var dow=(now.getDay()+6)%7;return{start:sod(nowT-dow*MS),end:nowT};} if(key==='last7')return{start:nowT-7*MS,end:nowT}; if(key==='last30')return{start:nowT-30*MS,end:nowT}; if(key==='qtd'){var qq=Math.floor(m/3);return{start:D(y,qq*3,1),end:nowT};} if(key==='ytd')return{start:D(y,0,1),end:nowT}; return {start:D(y,m,1),end:nowT}; }
    function renderList(){ var lc=q('lc'); if(!st.open){ lc.style.display='none'; lc.innerHTML=''; return; } var u; if(st.open==='__all__'){ var ar=[]; var addr=function(a){(a||[]).forEach(function(z){ar=ar.concat(z.records||[]);});}; addr(st.data&&st.data.stages); addr(st.data&&st.data.other); ar=ar.filter(fOpp); u={status:'__all__',label:'All Opportunities',count:ar.length,records:ar}; } else if(st.open==='__active__'||st.open==='__acquired__'){ var ar2=[]; var ad2=function(a){(a||[]).forEach(function(z){ar2=ar2.concat(z.records||[]);});}; ad2(st.data&&st.data.stages); ad2(st.data&&st.data.other); ar2=ar2.filter(fOpp); if(st.open==='__active__'){ ar2=ar2.filter(function(r){return r.status!=='ACQUIRED'&&r.status!=='NO_DEAL';}); } else { ar2=ar2.filter(function(r){return r.status==='ACQUIRED';}); } u={status:st.open,label:(st.open==='__active__'?'Active Deals':'Acquired'),count:ar2.length,records:ar2}; } else { u=units().filter(function(x){return x.status===st.open;})[0]; } if(!u){ lc.style.display='none'; lc.innerHTML=''; return; } var _lr=(u.records||[]).slice(); if(st.lcRep) _lr=_lr.filter(function(r){return (r.rep||'').trim()===st.lcRep;}); if(st.lcTF&&st.lcTF!=='all'){ var _lT=lcTfRange(st.lcTF); _lr=_lr.filter(function(r){ var _lt=Date.parse(r.stageEnteredAt); return !isNaN(_lt)&&_lt>=_lT.start&&_lt<_lT.end; }); } u={status:u.status,label:u.label,count:_lr.length,records:_lr}; var c=colByKey(st.sortK); var rows=(u.records||[]).slice().sort(function(a,b){ var av=sortVal(a,c),bv=sortVal(b,c); if(av<bv) return -1*st.sortDir; if(av>bv) return 1*st.sortDir; return 0; }); var th=COLS.map(function(col){ var ar=(st.sortK===col.k)?(st.sortDir>0?' ▲':' ▼'):''; return '<th data-k="'+col.k+'" class="'+(col.center||col.hcenter?'ctr':(col.num?'num':''))+(st.sortK===col.k?' on':'')+'">'+col.t+ar+'</th>'; }).join(''); var body=rows.length?rows.map(function(r){ return '<tr data-uuid="'+esc2(r.uuid)+'">'+COLS.map(function(col){ return '<td class="'+(col.center?'ctr':(col.num?'num':''))+'">'+cellHtml(r,col)+'</td>'; }).join('')+'</tr>'; }).join(''):'<tr><td colspan="15" style="color:#8a8a92;padding:12px;">No deals match.</td></tr>'; var _ini=function(nm){ return (nm||'').split(/\s+/).map(function(x){return x[0]||'';}).join('').slice(0,2).toUpperCase(); }; var _lcReps=(function(){var ss={},o=[];(u.records||[]).forEach(function(r){var v=(r.rep||'').trim();if(v&&!ss[v]){ss[v]=1;o.push(v);}});o.sort();return o;})(); var _LCTFO=[['all','All time'],['today','Today'],['thisweek','This week'],['mtd','Month to date'],['last7','Last 7 days'],['last30','Last 30 days'],['qtd','Quarter to date'],['ytd','Year to date']]; var _ltf=(st.lcTF||'all'); var _ltfLab=''; _LCTFO.forEach(function(o){ if(o[0]===_ltf&&_ltf!=='all') _ltfLab='<span class="bff-ddl">'+o[1]+'</span>'; }); var _tfSel='<div class="bff-dd bff-ddt'+(_ltf!=='all'?' bff-dd-act':'')+'"><button type="button" class="bff-ddb" data-ddtoggle="1" title="Timeframe"><i class="ti ti-clock-hour-4"></i>'+_ltfLab+'<i class="ti ti-chevron-down bff-ddcar"></i></button><div class="bff-ddm">'+_LCTFO.map(function(o){return '<div class="bff-ddo'+(o[0]===_ltf?' on':'')+'" data-dd="lc" data-kind="lctf" data-v="'+o[0]+'">'+o[1]+'</div>';}).join('')+'</div></div>'; var _rp=(st.lcRep||''); var _repFace=_rp?('<span class="bff-ddav">'+esc2(_ini(_rp))+'</span>'):'<i class="ti ti-user"></i>'; var _repSel='<div class="bff-dd bff-ddr'+(_rp?' bff-dd-act':'')+'"><button type="button" class="bff-ddb" data-ddtoggle="1" title="Rep">'+_repFace+'<i class="ti ti-chevron-down bff-ddcar"></i></button><div class="bff-ddm bff-ddmr"><div class="bff-ddo'+(_rp===''?' on':'')+'" data-dd="lc" data-kind="lcrep" data-v="">All Reps</div>'+_lcReps.map(function(rn){return '<div class="bff-ddo'+(_rp===rn?' on':'')+'" data-dd="lc" data-kind="lcrep" data-v="'+esc2(rn)+'"><span class="bff-ddav sm">'+esc2(_ini(rn))+'</span>'+esc2(rn)+'</div>';}).join('')+'</div></div>'; lc.style.display='block'; lc.innerHTML='<div class="bff-lc-h"><span class="bff-lc-t">'+esc2(u.label)+'</span><span class="bff-lc-c">'+u.count+' deal'+(u.count===1?'':'s')+' · click a column to sort</span><span class="bff-lc-ctrls">'+_repSel+_tfSel+'<button class="bff-filter bff-filter-lc" data-r="fbtn"><i class="ti ti-filter" style="color:#92df02;font-size:14px;" aria-hidden="true"></i>Filter</button></span></div><div class="bff-fpanel" data-r="fpanel" style="display:none;"></div><div class="bff-tblwrap"><table class="bff-tbl"><thead><tr>'+th+'</tr></thead><tbody>'+body+'</tbody></table></div>'; try{buildFilter();}catch(_e){} var _fb=lc.querySelector('[data-r="fbtn"]'); if(_fb) _fb.addEventListener('click',function(){ var fp=q('fpanel'); if(fp) fp.style.display=(fp.style.display==='none'?'block':'none'); }); [].forEach.call(lc.querySelectorAll('th'),function(h){ h.addEventListener('click',function(){ var k=h.getAttribute('data-k'); if(st.sortK===k){ st.sortDir=-st.sortDir; } else { st.sortK=k; st.sortDir=colByKey(k).num?-1:1; } renderList(); }); }); [].forEach.call(lc.querySelectorAll('.bff-tbtn'),function(b){ b.addEventListener('click',function(ev){ ev.stopPropagation(); var u3=b.getAttribute('data-link')||b.getAttribute('data-sheet'); if(u3){ try{ window.open(u3,'_blank','noopener'); }catch(e){} } }); }); [].forEach.call(lc.querySelectorAll('tbody tr'),function(tr){ tr.addEventListener('click',function(){ var u2=tr.getAttribute('data-uuid'); if(u2) openOverlay(u2); }); }); }
    function render(){ var F=q('funnel'); var U=units(); if(!U.length){ F.innerHTML='<div class="bff-empty">No data.</div>'; return; } var h=''; U.forEach(function(u,i){ if(i>0) h+='<div class="bff-conn"><span class="bff-dot"></span></div>'; var on=(st.open===u.status)?' on':''; var z=u.count===0?' zero':''; h+='<div class="bff-stage'+on+'" data-status="'+esc2(u.status)+'"><div class="bff-ico">'+pSvg(u.label)+'</div><div class="bff-n'+z+'">'+Number(u.count).toLocaleString('en-US')+'</div><div class="bff-l">'+esc2(u.label)+'</div></div>'; }); F.innerHTML=h; F.classList.toggle('bff-funnel-grid', st.view==='stages'); [].forEach.call(F.querySelectorAll('.bff-stage'),function(el){ el.addEventListener('click',function(){ var s2=el.getAttribute('data-status'); st.open=(st.open===s2?null:s2); st.lcTF='all'; st.lcRep=''; render(); renderList(); bfPipeKPI(); }); }); }
    ...
    [].forEach.call(R.querySelectorAll('.bff-tab'),function(t){ t.addEventListener('click',function(){ R.querySelectorAll('.bff-tab').forEach(function(x){x.classList.remove('on');}); t.classList.add('on'); st.view=t.getAttribute('data-view'); st.open=null; st.lcTF='all'; st.lcRep=''; refresh(); }); });

    var _cr=q('caret'); if(_cr) _cr.addEventListener('click',function(){ st.open=(st.open==='__all__'?null:'__all__'); st.lcTF='all'; st.lcRep=''; refresh(); });
  ...
  function bfIframeIsolate(){
    try{
      ...
      var el=rv;
      while(el && el!==document.body && el.parentElement){ var par=el.parentElement; [].forEach.call(par.children,function(ch){ if(ch!==el){ try{ ch.style.setProperty('display','none','important'); }catch(e){} } }); try{ par.style.setProperty('padding','0','important'); par.style.setProperty('margin','0','important'); if(emb){ par.style.setProperty('background','transparent','important'); par.style.setProperty('background-color','transparent','important'); } }catch(e){} el=par; }
      if(emb){ try{ [rv].forEach(function(x){ x.style.setProperty('background','transparent','important'); }); }catch(e){} }
      try{ document.body.style.setProperty('background', emb?'transparent':'#0c0d10','important'); }catch(e){}
