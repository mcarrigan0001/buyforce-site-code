...
(function () {
  ...
  function bfPipeCSS(){ if(window.__bfPipeCSS) return; window.__bfPipeCSS=1; var s=document.createElement('style'); s.textContent="#bff-wrap{font-family:Inter,system-ui,sans-serif;background:#0c0d10;border:1px solid #1d1f25;border-radius:16px;padding:20px 24px 16px;color:#e8e8ea;}#bff-wrap *{box-sizing:border-box;}.bff-head{display:flex;align-items:center;gap:14px;margin-bottom:8px;flex-wrap:wrap;}.bff-title{font-size:16px;font-weight:700;color:#fff;letter-spacing:-.2px;}.bff-tabs{display:flex;gap:6px;margin-left:auto;background:#16171b;border-radius:10px;padding:3px;}.bff-tab{font-size:12px;font-weight:600;color:#9a9aa2;padding:6px 14px;border-radius:8px;cursor:pointer;border:none;background:none;}.bff-tab.on{background:#26282e;color:#fff;}.bff-scope{font-size:11px;color:#7a7a82;font-weight:500;}.bff-funnel{display:flex;align-items:flex-start;gap:0;overflow-x:auto;padding:10px 0 6px;}.bff-stage{display:flex;flex-direction:column;align-items:center;flex:0 0 auto;min-width:118px;cursor:pointer;border-radius:12px;padding:6px 4px 8px;transition:background .12s;}.bff-stage:hover,.bff-stage.on{background:#141519;}.bff-ico{width:52px;height:52px;border-radius:50%;background:#15161b;border:1px solid #2a2c33;display:flex;align-items:center;justify-content:center;color:#d4d7db;font-size:23px;}.bff-n{font-size:33px;font-weight:800;color:#1fc77d;line-height:1;margin-top:13px;letter-spacing:-.5px;}.bff-n.zero{color:#54565c;}.bff-l{font-size:13px;color:#9aa0a6;margin-top:7px;text-align:center;white-space:nowrap;}.bff-conn{flex:1 1 22px;min-width:16px;height:64px;position:relative;display:flex;align-items:center;justify-content:center;}.bff-conn::before{content:'';position:absolute;left:-2px;right:-2px;top:32px;height:1px;background:linear-gradient(90deg,rgba(31,199,125,.14),rgba(31,199,125,.6),rgba(31,199,125,.14));}.bff-dot{width:9px;height:9px;border-radius:50%;background:#1fc77d;box-shadow:0 0 7px 1px rgba(31,199,125,.85);position:relative;margin-bottom:32px;}.bff-list{margin-top:12px;border-top:1px solid #1f2127;padding-top:14px;display:none;}.bff-list.show{display:block;}.bff-list-h{display:flex;align-items:center;gap:8px;margin-bottom:6px;}.bff-list-t{font-size:13px;font-weight:700;color:#fff;}.bff-list-c{font-size:12px;color:#8a8a92;}.bff-colh{display:flex;align-items:center;gap:12px;padding:6px 10px;font-size:10px;font-weight:700;letter-spacing:.5px;color:#6a6a72;text-transform:uppercase;}.bff-row{display:flex;align-items:center;gap:12px;padding:9px 10px;border-radius:9px;text-decoration:none;color:#dcdce2;cursor:pointer;border-top:1px solid #191b20;}.bff-row:hover{background:#141519;}.bff-veh{font-size:13px;font-weight:600;color:#f1f1f3;flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}.bff-sell{font-size:12px;color:#9a9aa2;flex:none;width:130px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}.bff-days{font-size:12px;flex:none;width:74px;text-align:right;font-weight:600;}.bff-act{font-size:12px;color:#9a9aa2;flex:none;width:84px;text-align:right;}.bff-open{flex:none;width:54px;text-align:right;color:#1fc77d;font-size:12px;font-weight:600;}.bff-empty,.bff-load{font-size:13px;color:#8a8a92;padding:10px 2px;}@media(max-width:640px){.bff-sell,.bff-act{display:none;}.bff-stage{min-width:92px;}.bff-conn{min-width:10px;}}"; document.head.appendChild(s); }
  ...
  function bfPipeRender(R){
    ...
    var st={view:'macro',data:null,open:null};
    R.innerHTML='<div id="bff-wrap"><div class="bff-head"><span class="bff-title">Pipeline Overview</span><span class="bff-scope" data-r="scope"></span><div class="bff-tabs"><button class="bff-tab" data-view="stages">All stages</button><button class="bff-tab on" data-view="macro">Macro</button></div></div><div class="bff-funnel" data-r="funnel"><div class="bff-load">Loading pipeline…</div></div><div class="bff-list" data-r="list"></div></div>';
    function q(n){ return R.querySelector('[data-r="'+n+'"]'); }
    function esc2(s){ return (s==null?'':String(s)).replace(/[&<>"]/g,function(c){return{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c];}); }
    function fmtNum(n){ try{ return Number(n).toLocaleString('en-US'); }catch(e){ return ''+n; } }
    ...
    function render(){ var F=q('funnel'); var U=units(); if(!U.length){ F.innerHTML='<div class="bff-empty">No data.</div>'; return; } var h=''; U.forEach(function(u,i){ if(i>0) h+='<div class="bff-conn"><span class="bff-dot"></span></div>'; var on=(st.open===u.status)?' on':''; var z=u.count===0?' zero':''; h+='<div class="bff-stage'+on+'" data-status="'+esc2(u.status)+'"><div class="bff-ico"><i class="ti '+pIcon(u.label)+'" aria-hidden="true"></i></div><div class="bff-n'+z+'">'+fmtNum(u.count)+'</div><div class="bff-l">'+esc2(u.label)+'</div></div>'; }); F.innerHTML=h; [].forEach.call(F.querySelectorAll('.bff-stage'),function(el){ el.addEventListener('click',function(){ var s2=el.getAttribute('data-status'); st.open=(st.open===s2?null:s2); render(); renderList(); }); }); renderList(); }
