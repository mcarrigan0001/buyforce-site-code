...
(function () {
  ...
  function bfWsConfirmText(deal, human){ var ui=STAGE_UI['Scheduled']; var t=''; if(ui&&ui.layout){ ui.layout.forEach(function(it){ if(it.k==='track'&&it.wt&&/confirm/i.test(it.wt.l)) t=it.wt.t; }); } if(!t) return ''; return t.replace(/\[First Name\]/g, (deal.sellerFirst||'there')).replace(/\[Appt Time\]/g, human).replace(/\[Dealership Address\]/g, (deal.address||'')).replace(/\[[^\]]+\]/g,'').trim(); }
  ...
  function bfWsHtml(stg, F){
    ...
    var pSH='<section class="bf-ws-panel" data-panel="schedule"><div class="bf-ws-lbl">Schedule the appointment</div><div class="bf-ws-sublbl">Date</div><div class="bf-ws-row">'+_days+'</div><div class="bf-ws-sublbl">Time</div><div class="bf-ws-row">'+_times+'</div><div class="bf-ws-sublbl">Location</div><div class="bf-ws-loc"><i class="ti ti-map-pin" aria-hidden="true"></i><div><div class="bf-ws-locname">Dealership</div><div class="bf-ws-locaddr">'+esc(_addr)+'</div></div></div><button class="bf-ws-btn primary" data-act="book" type="button" style="margin-top:14px;"><i class="ti ti-calendar-check" aria-hidden="true"></i>Book appointment</button><p class="bf-ws-micro">Stamps the time, moves the deal to <b>Scheduled</b>, and posts it to the Timeline.</p><div class="bf-ws-confirm"></div></section>';
  ...
  function bfWsAction(ws, uuid, act){
    ...
    if(act==='book'){ var dEl=ws.querySelector('.bf-ws-type.sel[data-day]'); var tEl=ws.querySelector('.bf-ws-type.sel[data-time]'); if(!dEl||!tEl){ bfToast('Pick a date and time'); return; } var day=dEl.getAttribute('data-day'); var tm=tEl.getAttribute('data-time'); var dObj=new Date(day+'T'+tm+':00'); var iso=isNaN(dObj.getTime())?'':dObj.toISOString(); var human=dEl.textContent.trim()+' at '+tEl.textContent.trim(); var deal=ws._bfDeal||{}; bfPost({uuid:uuid, status:'Scheduled', appointmentDateTime:iso}); bfPostEvent({uuid:uuid, type:'appt_booked', actor:actor, text:'Appointment booked for '+human}); bfWsPrepend(ws,{type:'appt_booked',actor:actor,text:'Appointment booked for '+human,createdAt:nowI}); bfToast('Booked \u00b7 moved to Scheduled'); var conf=ws.querySelector('.bf-ws-confirm'); if(conf){ var ctext=bfWsConfirmText(deal, human); conf.innerHTML='<div class="bf-ws-sublbl" style="margin-top:16px;">Confirmation text (send the evening before)</div><div class="bf-ws-track"><div class="bf-ws-trh"><span class="bf-ws-trn">Day-before reminder</span><button class="bf-ws-copy"><i class="ti ti-copy" aria-hidden="true"></i>Copy</button></div><div class="bf-ws-track-txt">'+esc(ctext)+'</div></div>'; } return; }
  ...
  function bfWorkspace(){
    ...
    ws.addEventListener('click', function(e){
      ...
      var ty=tg.closest&&tg.closest('.bf-ws-type'); if(ty){ var row=ty.closest('.bf-ws-row'); if(row){ row.querySelectorAll('.bf-ws-type').forEach(function(x){x.classList.toggle('sel',x===ty);}); } return; }
