...
(function () {
  ...
  function bfPipeCSS(){ if(window.__bfPipeCSS) return; window.__bfPipeCSS=1; var s=document.createElement('style'); s.textContent="#bff-wrap,.bff-listcard{font-family:Inter,system-ui,sans-serif;background:#0c0d10;border:1px solid #1d1f25;border-radius:16px;color:#e8e8ea;}#bff-wrap{padding:20px 24px 16px;}#bff-wrap *,.bff-listcard *{box-sizing:border-box;}.bff-listcard{margin-top:16px;padding:16px 18px;display:none;}.bff-head{display:flex;align-items:center;gap:12px;margin-bottom:8px;flex-wrap:wrap;}.bff-title{font-size:17px;font-weight:700;color:#fff;letter-spacing:-.2px;}.bff-glow{color:#fff;text-shadow:0 0 8px rgba(87,200,34,.95),0 0 20px rgba(87,200,34,.6);}.bff-scope{font-size:11px;color:#7a7a82;font-weight:500;}.bff-rt{margin-left:auto;display:flex;align-items:center;gap:8px;}.bff-tabs{display:flex;gap:6px;background:#16171b;border-radius:10px;padding:3px;}.bff-tab{font-size:12px;font-weight:600;color:#9a9aa2;padding:6px 14px;border-radius:8px;cursor:pointer;border:none;background:none;}.bff-tab.on{background:#26282e;color:#fff;}.bff-fwrap{position:relative;}.bff-filter{display:inline-flex;align-items:center;gap:6px;background:#15161b;border:1px solid #2a2c33;color:#e8e8ea;font-size:12px;font-weight:600;padding:7px 13px;border-radius:9px;cursor:pointer;}.bff-filter:hover{background:#1b1d22;}.bff-fdrop{position:absolute;right:0;top:38px;z-index:50;background:#15161b;border:1px solid #2a2c33;border-radius:10px;padding:5px;min-width:170px;box-shadow:0 8px 24px rgba(0,0,0,.4);}.bff-fitem{font-size:13px;color:#cfd2d6;padding:8px 10px;border-radius:7px;cursor:pointer;}.bff-fitem:hover{background:#22242a;}.bff-funnel{display:flex;align-items:flex-start;gap:0;overflow-x:auto;padding:10px 0 6px;}.bff-stage{display:flex;flex-direction:column;align-items:center;flex:0 0 auto;min-width:118px;cursor:pointer;border-radius:12px;padding:6px 4px 8px;transition:background .12s;}.bff-stage:hover,.bff-stage.on{background:#141519;}.bff-ico{width:52px;height:52px;border-radius:50%;background:#15161b;border:1px solid #2a2c33;display:flex;align-items:center;justify-content:center;color:#d4d7db;font-size:23px;}.bff-n{font-size:33px;font-weight:800;color:#57c822;line-height:1;margin-top:13px;letter-spacing:-.5px;}.bff-n.zero{color:#54565c;}.bff-l{font-size:13px;color:#9aa0a6;margin-top:7px;text-align:center;white-space:nowrap;}.bff-conn{flex:1 1 22px;min-width:16px;height:64px;position:relative;}.bff-conn::before{content:'';position:absolute;left:-2px;right:-2px;top:32px;height:1px;background:linear-gradient(90deg,rgba(87,200,34,.14),rgba(87,200,34,.6),rgba(87,200,34,.14));}.bff-dot{position:absolute;left:50%;top:32px;width:9px;height:9px;border-radius:50%;background:#57c822;box-shadow:0 0 7px 1px rgba(87,200,34,.85);transform:translate(-50%,-50%);}.bff-lc-h{display:flex;align-items:center;gap:8px;margin-bottom:10px;}.bff-lc-t{font-size:14px;font-weight:700;color:#fff;}.bff-lc-c{font-size:12px;color:#8a8a92;}.bff-tblwrap{overflow-x:auto;}.bff-tbl{width:100%;border-collapse:collapse;font-size:12.5px;}.bff-tbl th{text-align:left;font-size:10px;font-weight:700;letter-spacing:.4px;color:#7a7c84;text-transform:uppercase;padding:7px 10px;cursor:pointer;white-space:nowrap;border-bottom:1px solid #1f2127;user-select:none;}.bff-tbl th:hover{color:#b6b9c0;}.bff-tbl th.on{color:#57c822;}.bff-tbl th.num{text-align:right;}.bff-tbl td{padding:9px 10px;border-bottom:1px solid #161820;color:#dcdce2;white-space:nowrap;}.bff-tbl td.num{text-align:right;}.bff-tbl tbody tr{cursor:pointer;}.bff-tbl tbody tr:hover{background:#141519;}.bff-tbl .veh{font-weight:600;color:#f1f1f3;}.bff-empty,.bff-load{font-size:13px;color:#8a8a92;padding:10px 2px;}@media(max-width:640px){.bff-stage{min-width:92px;}.bff-conn{min-width:10px;}}"; document.head.appendChild(s); }
  ...
  function bfPipeRender(R){
    ...
    var MACRO=[{label:'Fresh Leads',statuses:['FRESH_LEADS']},{label:'Capturing VIN',statuses:['ENGAGED_AWAITING_VIN']},{label:'Appraising',statuses:['VIN_RECEIVED_APPRAISAL_NEEDED','APPRAISAL_COMPLETE_ENTER_OFFER_SHEET_VALUES','APPRAISAL_REVIEW_NEEDED','APPRAISAL_REVIEW_COMPLETE']},{label:'Fresh Offer Sent',statuses:['OFFER_SHEET_GENERATED','OFFER_SENT_0_2_DAYS']},{label:'Follow Up',statuses:['NURTURING_FOLLOW_UP_AND_RE_ENGAGE','VERBAL_YES_SCHEDULE_APPT','SCHEDULED','APPT_SHOWN_FOLLOW_UP']},{label:'Acquired',statuses:['ACQUIRED']},{label:'No Deal',statuses:['NO_DEAL']}];
    var COLS=[{k:'title',t:'Vehicle'},{k:'seller',t:'Seller'},{k:'daysInStage',t:'Days in Stage',num:true},{k:'asking',t:'Asking',num:true,money:true},{k:'offer',t:'Offer',num:true,money:true},{k:'acv',t:'ACV',num:true,money:true},{k:'competition',t:'Competition'},{k:'rep',t:'Assigned Rep'},{k:'mileage',t:'Mileage',num:true},{k:'driveTime',t:'Drive Time',num:true,drive:true},{k:'offerStatus',t:'Offer Status'}];
    var st={view:'macro',data:null,open:null,sortK:'daysInStage',sortDir:-1,dealId:null};
    R.innerHTML='<div id="bff-wrap"><div class="bff-head"><span class="bff-title bff-glow">Pipeline Summary</span><span class="bff-scope" data-r="scope"></span><div class="bff-rt"><div class="bff-fwrap" data-r="fwrap" style="display:none;"><button class="bff-filter" data-r="fbtn"><i class="ti ti-plus" style="color:#57c822;font-size:15px;" aria-hidden="true"></i><span data-r="flabel">Filter</span></button><div class="bff-fdrop" data-r="fdrop" style="display:none;"></div></div><div class="bff-tabs"><button class="bff-tab" data-view="stages">All stages</button><button class="bff-tab on" data-view="macro">Macro</button></div></div></div><div class="bff-funnel" data-r="funnel"><div class="bff-load">Loading pipeline…</div></div></div><div class="bff-listcard" data-r="lc"></div>';
    function q(n){ return R.querySelector('[data-r="'+n+'"]'); }
    function esc2(s){ return (s==null?'':String(s)).replace(/[&<>"]/g,function(c){return{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c];}); }
    function usd(n){ if(n==null||n==='') return '—'; var x=Number(n); if(isNaN(x)) return '—'; return '$'+Math.round(x).toLocaleString('en-US'); }
    ...
    function compColor(c){ c=(c||'').toLowerCase(); if(c.indexOf('both')>-1) return '#57c822'; if(c.indexOf('✓')>-1||c.indexOf('beats c')>-1) return '#9fd86a'; if(c.indexOf('neither')>-1||c.indexOf('✗')>-1) return '#c97a7a'; return '#cfd2d6'; }
    ...
    function cellHtml(r,c){ var v=r[c.k]; if(c.k==='title') return '<span class="veh">'+(esc2(v)||'(untitled)')+'</span>'; if(c.k==='daysInStage') return v==null?'—':('<span style="color:'+daysColor(v)+';font-weight:600;">'+v+'d</span>'); if(c.money) return usd(v); if(c.k==='mileage') return num(v); if(c.k==='competition') return v?('<span style="color:'+compColor(v)+';">'+esc2(v)+'</span>'):'—'; if(c.k==='offerStatus') return v?esc2(String(v).replace(/_/g,' ').toLowerCase()):'—'; if(c.k==='driveTime') return v?esc2(v):'—'; return esc2(v)||'—'; }
    function colByKey(k){ for(var i=0;i<COLS.length;i++){ if(COLS[i].k===k) return COLS[i]; } return COLS[2]; }
    function units(){ var s=(st.data&&st.data.stages)||[]; if(st.view==='stages'){ return s.concat((st.data&&st.data.other)||[]); } var by={}; s.forEach(function(x){by[x.status]=x;}); return MACRO.map(function(m){ var recs=[],c=0; m.statuses.forEach(function(cd){ var z=by[cd]; if(z){ c+=z.count; recs=recs.concat(z.records||[]); } }); return {status:m.label,label:m.label,count:c,records:recs}; }); }
    function renderList(){ var lc=q('lc'); if(!st.open){ lc.style.display='none'; lc.innerHTML=''; return; } var u=units().filter(function(x){return x.status===st.open;})[0]; if(!u){ lc.style.display='none'; lc.innerHTML=''; return; } var c=colByKey(st.sortK); var rows=(u.records||[]).slice().sort(function(a,b){ var av=sortVal(a,c),bv=sortVal(b,c); if(av<bv) return -1*st.sortDir; if(av>bv) return 1*st.sortDir; return 0; }); var th=COLS.map(function(col){ var ar=(st.sortK===col.k)?(st.sortDir>0?' ▲':' ▼'):''; return '<th data-k="'+col.k+'" class="'+(col.num?'num':'')+(st.sortK===col.k?' on':'')+'">'+col.t+ar+'</th>'; }).join(''); var body=rows.length?rows.map(function(r){ return '<tr data-uuid="'+esc2(r.uuid)+'">'+COLS.map(function(col){ return '<td class="'+(col.num?'num':'')+'">'+cellHtml(r,col)+'</td>'; }).join('')+'</tr>'; }).join(''):'<tr><td colspan="11" style="color:#8a8a92;padding:12px;">No deals in this stage.</td></tr>'; lc.style.display='block'; lc.innerHTML='<div class="bff-lc-h"><span class="bff-lc-t">'+esc2(u.label)+'</span><span class="bff-lc-c">'+u.count+' deal'+(u.count===1?'':'s')+' · click a column to sort</span></div><div class="bff-tblwrap"><table class="bff-tbl"><thead><tr>'+th+'</tr></thead><tbody>'+body+'</tbody></table></div>'; [].forEach.call(lc.querySelectorAll('th'),function(h){ h.addEventListener('click',function(){ var k=h.getAttribute('data-k'); if(st.sortK===k){ st.sortDir=-st.sortDir; } else { st.sortK=k; st.sortDir=colByKey(k).num?-1:1; } renderList(); }); }); [].forEach.call(lc.querySelectorAll('tbody tr'),function(tr){ tr.addEventListener('click',function(){ var u2=tr.getAttribute('data-uuid'); if(u2) window.location.href=RECORD_BASE+u2; }); }); }
    function render(){ var F=q('funnel'); var U=units(); if(!U.length){ F.innerHTML='<div class="bff-empty">No data.</div>'; return; } var h=''; U.forEach(function(u,i){ if(i>0) h+='<div class="bff-conn"><span class="bff-dot"></span></div>'; var on=(st.open===u.status)?' on':''; var z=u.count===0?' zero':''; h+='<div class="bff-stage'+on+'" data-status="'+esc2(u.status)+'"><div class="bff-ico"><i class="ti '+pIcon(u.label)+'" aria-hidden="true"></i></div><div class="bff-n'+z+'">'+Number(u.count).toLocaleString('en-US')+'</div><div class="bff-l">'+esc2(u.label)+'</div></div>'; }); F.innerHTML=h; [].forEach.call(F.querySelectorAll('.bff-stage'),function(el){ el.addEventListener('click',function(){ var s2=el.getAttribute('data-status'); st.open=(st.open===s2?null:s2); render(); renderList(); }); }); }
    function buildFilter(){ var fw=q('fwrap'); if(!fw) return; if(!(st.data&&st.data.canFilter)){ fw.style.display='none'; return; } fw.style.display='inline-flex'; var dd=q('fdrop'); var items='<div class="bff-fitem" data-id="">All dealerships</div>'; (st.data.dealerships||[]).forEach(function(dl){ items+='<div class="bff-fitem" data-id="'+dl.id+'">'+esc2(dl.name)+'</div>'; }); dd.innerHTML=items; [].forEach.call(dd.querySelectorAll('.bff-fitem'),function(it){ it.addEventListener('click',function(){ var idv=it.getAttribute('data-id'); dd.style.display='none'; load(idv===''?null:(+idv)); }); }); var fl=q('flabel'); var sel=st.data.selectedDealershipId; var nm=''; if(sel!=null){ (st.data.dealerships||[]).forEach(function(dl){ if(dl.id===sel) nm=dl.name; }); } fl.textContent=nm||'Filter'; }
    ...
    function load(dealId){ st.dealId=(dealId!=null?dealId:null); var tk=''; try{ tk=bfGetApiToken(); }catch(e){} var bo=st.dealId!=null?{dealershipId:st.dealId}:{}; fetch(ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json','X-BF-Token':tk},body:JSON.stringify(bo)}).then(function(r){return r.json();}).then(function(d){ st.data=d||{stages:[]}; updateScope(d); buildFilter(); render(); renderList(); }).catch(function(){ q('funnel').innerHTML='<div class="bff-empty">Could not load the pipeline. Refresh to retry.</div>'; }); }
    [].forEach.call(R.querySelectorAll('.bff-tab'),function(t){ t.addEventListener('click',function(){ R.querySelectorAll('.bff-tab').forEach(function(x){x.classList.remove('on');}); t.classList.add('on'); st.view=t.getAttribute('data-view'); st.open=null; render(); renderList(); }); });
    (function(){ var fb=q('fbtn'); if(fb) fb.addEventListener('click',function(ev){ ev.stopPropagation(); var dd=q('fdrop'); dd.style.display=(dd.style.display==='none'?'block':'none'); }); document.addEventListener('click',function(){ var dd=q('fdrop'); if(dd) dd.style.display='none'; }); })();
    load(null);
  }
