...
(function () {
  ...
  function bfPipeRender(R){ try{ R.style.visibility='visible'; if(window.__bfStopPrehide) window.__bfStopPrehide(); }catch(_e){}
    ...
    function openOverlay(uuid){ try{ sessionStorage.setItem('bf.pipeorder', JSON.stringify(navList())); }catch(_o){}
      function _bfClk(){ try{ var _na=document.querySelector('[data-testid="collection-record"][href*="'+uuid+'"]'); if(_na){ _na.click(); return true; } }catch(_e){} return false; }
      if(_bfClk()) return;
      var _t=0; if(st.__scrollT) clearInterval(st.__scrollT);
      st.__scrollT=setInterval(function(){ _t++; try{ [].forEach.call(document.querySelectorAll('[data-testid="collection-group"] [class*="overflow-y-auto"], [data-testid="view-collection"] [class*="overflow-y-auto"]'), function(sc){ sc.scrollTop=sc.scrollHeight; }); }catch(e){} if(_bfClk()){ clearInterval(st.__scrollT); st.__scrollT=0; return; } if(_t>18){ clearInterval(st.__scrollT); st.__scrollT=0; ensureOverlay(); st.ovlOpenedAt=Date.now(); ovlList=navList(); ovlIdx=ovlList.indexOf(uuid); if(ovlIdx<0){ ovlList=[uuid]; ovlIdx=0; } ovl.style.display='block'; ovlShow(); } }, 160);
    }
