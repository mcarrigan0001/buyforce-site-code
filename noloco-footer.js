...
(function () {
  ...
  function bfFlipGo(u){
    ...
    try{
      var _url;
      if(/\/(preview|view)\/rec[0-9a-z]+/i.test(location.pathname)){ _url=location.pathname.replace(/rec[0-9a-z]+/i, u); }
      else { var _p=location.pathname.split('/'); _url='/'+(_p[1]||'opportunities-1')+'/preview/'+u+'/overview'; }
      if(/\/rec[0-9a-z]+$/i.test(_url)) _url+='/overview';
