...
(function () {
  ...
  try{ if(!window.__bfProbe){ window.__bfProbe=1; setTimeout(function(){ var o={}; try{ o.lambda=localStorage.getItem('noloco.lambda-endpoint'); }catch(e){} try{ o.lambdaMut=localStorage.getItem('noloco.lambda-mutation-endpoint'); }catch(e){} try{ o.ajsUserId=localStorage.getItem('ajs_user_id'); }catch(e){} try{ var ca=localStorage.getItem('noloco-client-auth'); if(ca){ var pj=null; try{ pj=JSON.parse(ca); }catch(e2){} if(pj&&typeof pj==='object'){ o.authType='json'; o.authKeys=Object.keys(pj); var usr=pj.user||pj.currentUser||pj.viewer||null; if(usr&&typeof usr==='object'){ o.userKeys=Object.keys(usr); o.userId=usr.id; o.hasDealership=('dealership' in usr)||('dealershipId' in usr); o.hasApiToken=('apiToken' in usr); } var t=pj.token||pj.accessToken||pj.authToken||pj.jwt; o.hasToken=!!t; } else { o.authType='string'; o.authLen=ca.length; o.looksJwt=(ca.split('.').length===3); } } }catch(e){ o.caErr=String(e); } console.log('BF_PROBE2 '+JSON.stringify(o)); }, 3500); } }catch(e){}
