...
(function () {
  ...
  function bfCaptureApiToken(){ try{ if(bfGetApiToken()){ try{ document.querySelectorAll('[data-bftok]').forEach(function(e){e.style.display='none';}); }catch(e){} if(window.__bfTokTimer){ clearInterval(window.__bfTokTimer); window.__bfTokTimer=null; } return; } if(bfTokTries===0){ try{ console.log('BF_SCAN active (v2)'); }catch(e){} } if(bfTokTries++>40){ try{ console.log('BF_SCAN gave up'); }catch(e){} if(window.__bfTokTimer){ clearInterval(window.__bfTokTimer); window.__bfTokTimer=null; } return; } var rx=/bft_[A-Za-z0-9]{20,}/; var done=function(tok,el){ try{ sessionStorage.setItem('bf.apitoken', tok); }catch(e){} try{ if(el){ el.setAttribute('data-bftok','1'); el.style.display='none'; } }catch(e){} try{ console.log('BF_TOKEN captured ok'); bfToast('BF auth ready \u2713'); }catch(e){} if(window.__bfTokTimer){ clearInterval(window.__bfTokTimer); window.__bfTokTimer=null; } }; var els=document.querySelectorAll('p,span,div,td,h1,h2,h3,h4,h5,h6,a,label,li,strong,em,b'); for(var i=0;i<els.length;i++){ var el=els[i]; if(el.children.length===0){ var m=(el.textContent||'').match(rx); if(m){ done(m[0], el); return; } } } var ins=document.querySelectorAll('input,textarea'); for(var j=0;j<ins.length;j++){ var v=(ins[j].value||'')+''; var m2=v.match(rx); if(m2){ done(m2[0], ins[j]); return; } } }catch(e){} }
