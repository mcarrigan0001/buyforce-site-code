...
(function () {
  ...
  function bfPipeRender(R){
    ...
    function bfPipeKPI(){
      ...
      var REPL={conversion:'Conversion Rate',vehicles:'Vehicles by Stage',avgdaysstage:'Avg Days in Stage',acquisition:'Acquisition Rate',appraisalcomp:'Appraisal Completion',offeracc:'Offer Acceptance',followup:'Follow-up Effectiveness',velocity:'Acquisition Velocity',appraiser:'Appraiser Performance'};
      ...
      function barsHtml(items,fmt){ var mx=Math.max.apply(null,items.map(function(x){return x.v;}).concat([1])); return '<div class="bff-rbars">'+items.map(function(x){ var h=Math.max(3,Math.round((x.v/mx)*92)); return '<div class="bff-rcol"><div class="bff-rbar-v">'+(fmt?fmt(x.v):x.v)+'</div><div class="bff-rbar" style="height:'+h+'px;"></div><div class="bff-rbar-l">'+esc2(x.l)+'</div></div>'; }).join('')+'</div>'; }
      function apprReport(){ var by={}; recs.forEach(function(r){ var ap=(r.appraiser||'').trim(); if(!ap) return; if(!by[ap]) by[ap]={asg:0,comp:0,bCM:0,bCV:0,bBoth:0,bNeither:0,cmD:0,cvD:0,bothD:0,days:0,daysN:0}; var a=by[ap]; a.asg++; if(ord(r.status)>=3){ a.comp++; if(r.daysInStage!=null){ a.days+=Number(r.daysInStage||0); a.daysN++; } } var cm=Number(r.carmax),cv=Number(r.carvana),off=Number(r.offer)||Number(r.acv); var hCM=!isNaN(cm)&&cm>0,hCV=!isNaN(cv)&&cv>0; if(hCM){a.cmD++; if(off>cm)a.bCM++;} if(hCV){a.cvD++; if(off>cv)a.bCV++;} if(hCM&&hCV){a.bothD++; if(off>cm&&off>cv)a.bBoth++; if(!(off>cm)&&!(off>cv))a.bNeither++;} }); var ks=Object.keys(by); if(!ks.length) return '<div class="bff-empty">No appraiser data yet \u2014 assign appraisers on deals to populate this.</div>'; function p(n,d){return d?Math.round(n/d*100):0;} var h='<div class="bff-rtblwrap"><table class="bff-rtbl"><thead><tr><th>Appraiser</th><th class="n">Assigned</th><th class="n">Completed</th><th class="n">Compl %</th><th class="n">Avg Days</th><th class="n">Beat CarMax</th><th class="n">Beat Carvana</th><th class="n">Beat Both</th><th class="n">Beat Neither</th></tr></thead><tbody>'; h+=ks.map(function(k){var a=by[k]; return '<tr><td>'+esc2(k)+'</td><td class="n">'+a.asg+'</td><td class="n">'+a.comp+'</td><td class="n">'+p(a.comp,a.asg)+'%</td><td class="n">'+(a.daysN?Math.round(a.days/a.daysN*10)/10:'\u2014')+'</td><td class="n">'+p(a.bCM,a.cmD)+'%</td><td class="n">'+p(a.bCV,a.cvD)+'%</td><td class="n">'+p(a.bBoth,a.bothD)+'%</td><td class="n">'+p(a.bNeither,a.bothD)+'%</td></tr>';}).join(''); return h+'</tbody></table></div>'; }
      function bfPipeReports(){ var which=st.tf.report||'conversion'; var U=units(); function reached(i){ var s2=0; for(var k=i;k<U.length;k++) s2+=U[k].count; return s2; } var html=''; if(which==='vehicles'){ html=barsHtml(U.map(function(u){return {l:u.label,v:u.count};})); } else if(which==='conversion'){ html=barsHtml(U.map(function(u,i){ return {l:u.label,v:(i===0?100:pctv(reached(i),reached(i-1)))}; }),function(v){return v+'%';}); } else if(which==='acquisition'){ var ra=reached(U.length-1); html=barsHtml(U.map(function(u,i){return {l:u.label,v:pctv(ra,reached(i))};}),function(v){return v+'%';}); } else if(which==='avgdaysstage'){ html=barsHtml(U.map(function(u){ var fr=u.records.filter(function(r){return r.daysInStage!=null;}); var a=fr.length?Math.round(fr.reduce(function(s2,r){return s2+Number(r.daysInStage||0);},0)/fr.length*10)/10:0; return {l:u.label,v:a}; }),function(v){return v+'d';}); } else if(which==='appraisalcomp'){ var vinR=reached(1),apR=reached(2); html='<div class="bff-rdonut">'+pctv(apR,vinR)+'%</div><div class="bff-rsub">Appraisals completed \u00b7 '+apR+' of '+vinR+' VINs captured</div>'; } else if(which==='offeracc'){ var ofR=reached(3),acR=reached(U.length-1); html='<div class="bff-rdonut">'+pctv(acR,ofR)+'%</div><div class="bff-rsub">Offers accepted \u00b7 '+acR+' of '+ofR+' offers sent</div>'; } else if(which==='velocity'){ var aq=recs.filter(function(r){return r.status==='ACQUIRED'&&r.dateListed&&r.stageEnteredAt;}); var dd=aq.map(function(r){return (Date.parse(r.stageEnteredAt)-Date.parse(r.dateListed))/864e5;}).filter(function(x){return !isNaN(x)&&x>=0;}); var av=dd.length?Math.round(dd.reduce(function(s2,x){return s2+x;},0)/dd.length*10)/10:0; html='<div class="bff-rdonut">'+av+'<span class="bff-bigunit"> days</span></div><div class="bff-rsub">Average lead \u2192 acquisition \u00b7 '+dd.length+' deals</div>'; } else if(which==='followup'){ var bk={}; recs.forEach(function(r){ var t=Math.min(4,Number(r.followups||0)); if(!bk[t])bk[t]={tot:0,acq:0}; bk[t].tot++; if(r.status==='ACQUIRED')bk[t].acq++; }); html=barsHtml([0,1,2,3,4].map(function(t){ var b=bk[t]||{tot:0,acq:0}; return {l:(t===4?'4+':String(t)),v:pctv(b.acq,b.tot)}; }),function(v){return v+'%';})+'<div class="bff-rsub">Acquisition rate by number of follow-up touches</div>'; } else if(which==='appraiser'){ html=apprReport(); } var rEl=q('report'); if(rEl) rEl.innerHTML='<div class="bff-ph"><span>'+REPL[which]+'</span>'+reportSel()+'</div><div class="bff-rbody">'+html+'</div>'; }
      bfPipeReports();
