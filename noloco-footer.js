...
(function () {
  ...
  function bfFlipGo(u){ location.href='/opportunities/view/'+u+'/summary'; }
  /* ---- v2 record page: native board/kanban card clicks open the v2 dark record (Noloco routes via React onClick, so intercept in capture phase) ---- */
  function bfV2SetOrder(){ try{ var us=[]; document.querySelectorAll('a[data-testid="collection-record"]').forEach(function(c){ var mm=(c.getAttribute('href')||'').match(/(rec[0-9a-z]+)/i); if(mm && us.indexOf(mm[1])<0) us.push(mm[1]); }); if(us.length) sessionStorage.setItem('bf.pipeorder', JSON.stringify(us)); }catch(e){} }
  document.addEventListener('click', function(e){
    if(e.button!==0 || e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) return;
    var t=e.target; if(!t||!t.closest) return;
    var card=t.closest('a[data-testid="collection-record"]'); if(!card) return;
    if(t.closest('.bf-ms,.bf-comment,.bf-listing,.bf-vincopy,.bf-fedit,.bf-amt,.bf-collapse-bar,.bf-undo,.bf-openrec,.bf-btn,.bf-pill,.bf-wt,[data-editing],button,input,select,textarea,[role="button"],a:not([data-testid="collection-record"])')) return;
    var m=(card.getAttribute('href')||'').match(/(rec[0-9a-z]+)/i); if(!m) return;
    e.preventDefault(); e.stopPropagation();
    bfV2SetOrder();
    location.href='/opportunities/view/'+m[1]+'/summary';
  }, true);
  ...
  function bfPipeGo(u){ location.href='/opportunities/view/'+u+'/summary'; }
  ...
  document.addEventListener('click', function(e){
    ...
    if(openr){ e.preventDefault(); e.stopPropagation(); var oc=openr.closest('[data-testid="collection-record"]'); var oh=oc?oc.getAttribute('href'):''; var om=oh.match(/(rec[0-9a-z]+)/i); if(om){ location.href='/opportunities/view/'+om[1]+'/summary'; } return; }
  ...
  function bfPipeRender(R){ try{ R.style.visibility='visible'; if(window.__bfStopPrehide) window.__bfStopPrehide(); }catch(_e){}
    ...
    function openOverlay(uuid){ try{ sessionStorage.setItem('bf.pipeorder', JSON.stringify(navList())); }catch(_o){} location.href='/opportunities/view/'+uuid+'/summary'; }
