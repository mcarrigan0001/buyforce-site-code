...
(function () {
  ...
  function bfCaptureApiToken(){ try{ if(bfGetApiToken()){ try{ document.querySelectorAll('[data-bftok]').forEach(function(e){e.style.display='none';}); }catch(e){} if(window.__bfTokTimer){ clearInterval(window.__bfTokTimer); window.__bfTokTimer=null; } return; } if(bfTokTries===0){ try{ console.log('BF_SCAN active (v3)'); }catch(e){} } if(bfTokTries++>60){ try{ console.log('BF_SCAN gave up'); }catch(e){} if(window.__bfTokTimer){ clearInterval(window.__bfTokTimer); window.__bfTokTimer=null; } return; } var rx=/bft_[A-Za-z0-9]{20,}/; var done=function(tok,el){ try{ sessionStorage.setItem('bf.apitoken', tok); }catch(e){} try{ if(el){ if(el.setAttribute) el.setAttribute('data-bftok','1'); if(el.style) el.style.display='none'; } }catch(e){} try{ console.log('BF_TOKEN captured ok'); bfToast('BF auth ready \u2713'); }catch(e){} if(window.__bfTokTimer){ clearInterval(window.__bfTokTimer); window.__bfTokTimer=null; } }; try{ var tw=document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null); var tn; while(tn=tw.nextNode()){ var mm=(tn.nodeValue||'').match(rx); if(mm){ done(mm[0], tn.parentElement); return; } } }catch(e){} try{ var ins=document.querySelectorAll('input,textarea'); for(var j=0;j<ins.length;j++){ var v=(ins[j].value||'')+''; var m2=v.match(rx); if(m2){ done(m2[0], ins[j]); return; } } }catch(e){} }
