...
(function () {
  ...
  function bfPipeGo(u){ var url='/command/view/'+u+'/overview'+location.search; try{ history.pushState({},'',url); window.dispatchEvent(new PopStateEvent('popstate')); }catch(e){ location.href=url; } }
  ...
  function bfPipeRender(R){ try{ R.style.visibility='visible'; if(window.__bfStopPrehide) window.__bfStopPrehide(); }catch(_e){}
    ...
    var RECORD_BASE='https://buyforce.noloco.co/command/view/';
    ...
    function openOverlay(uuid){ try{ sessionStorage.setItem('bf.pipeorder', JSON.stringify(navList())); }catch(_o){} try{ var _na=document.querySelector('[data-testid="collection-record"][href*="'+uuid+'"]'); if(_na){ window.__bfPipeDrawerOpenedAt=Date.now(); try{ sessionStorage.setItem('bf.pipesnap', R.innerHTML); }catch(_s){} _na.click(); return; } }catch(_e){} /* never open the iframe overlay: navigate to the native record route the board uses */ try{ var _tmpl=document.querySelector('[data-testid="collection-record"][href*="/preview/"],[data-testid="collection-record"][href*="/view/"]'); var _url; if(_tmpl){ _url=(_tmpl.getAttribute('href')||'').replace(/rec[0-9a-z]+/i, uuid); if(/\/rec[0-9a-z]+$/i.test(_url)) _url+='/overview'; } else { var _base=location.pathname.split('/')[1]||'command'; _url='/'+_base+'/preview/'+uuid+'/overview'; } window.__bfPipeDrawerOpenedAt=Date.now(); try{ history.pushState({},'',_url); window.dispatchEvent(new PopStateEvent('popstate')); }catch(_pe){ location.href=_url; } }catch(_e2){ location.href='/command/preview/'+uuid+'/overview'; } }
  ...
  function bfPipelinePage(){ try{ if(!window.__bfPipeLog){ window.__bfPipeLog=1; try{ console.log('BF_PIPE active'); }catch(e){} } bfPipeTitle(); if(/^\/command(\/|$)/.test(location.pathname)){ var vc=document.querySelector('[data-testid="view-collection"]'); if(vc){ var ph=document.getElementById('bf-pipe-host'); if(!ph){ ph=document.createElement('div'); ph.id='bf-pipe-host'; ph.setAttribute('data-bfpipe','1'); ph.style.cssText='display:flex;flex-direction:column;flex:1 1 auto;min-height:0;align-self:stretch;width:auto;min-width:0;max-width:100%;'; vc.insertBefore(ph, vc.firstChild); bfPipeCSS(); bfPipeRender(ph); try{ console.log('BF_PIPE collection host rendered'); }catch(e){} } else if(ph.parentElement!==vc){ vc.insertBefore(ph, vc.firstChild); } try{ if(ph){ var _bfSzH=function(){ var _vr=vc.getBoundingClientRect(); if(_vr.width<=100) return; var _w=Math.round(_vr.width*0.85); ph.style.setProperty('width',_w+'px','important'); ph.style.setProperty('max-width','none','important'); ph.style.setProperty('margin-left','0','important'); ph.style.setProperty('margin-right','0','important'); ph.style.setProperty('transform','none','important'); var _hl=ph.getBoundingClientRect().left; var _dx=Math.round(_vr.left-_hl)+Math.round((_vr.width-_w)/2); ph.style.setProperty('transform','translateX('+_dx+'px)','important'); ph.__bfW=_w; }; var _vr0=vc.getBoundingClientRect(); var _wt=(_vr0.width>100)?Math.round(_vr0.width*0.85):0; if(_wt&&ph.__bfW!==_wt){ _bfSzH(); } if(!window.__bfPipeResize){ window.__bfPipeResize=1; var _rt; window.addEventListener('resize',function(){ clearTimeout(_rt); _rt=setTimeout(function(){ try{ var p2=document.getElementById('bf-pipe-host'),v2=document.querySelector('[data-testid="view-collection"]'); if(p2&&v2){ var vv=v2.getBoundingClientRect(); if(vv.width>100){ var ww=Math.round(vv.width*0.85); p2.style.setProperty('width',ww+'px','important'); p2.style.setProperty('transform','none','important'); var hl=p2.getBoundingClientRect().left; var dx=Math.round(vv.left-hl)+Math.round((vv.width-ww)/2); p2.style.setProperty('transform','translateX('+dx+'px)','important'); p2.__bfW=ww; } } }catch(e){} },120); },{passive:true}); } } }catch(_w){} var coll=vc.querySelector('.collection-view'); if(coll) coll.style.setProperty('display','none','important'); [].forEach.call(vc.children,function(c){ if(c===ph) return; if(c.classList&&c.classList.contains('collection-view')) return; if(c.querySelector&&c.querySelector('[data-testid="record-view"]')) return; if(c.getAttribute&&c.getAttribute('data-testid')==='record-view') return; c.style.setProperty('display','none','important'); }); if((location.pathname==='/command') && window.__bfPipeDrawerOpenedAt){ var _oa=window.__bfPipeDrawerOpenedAt; window.__bfPipeDrawerOpenedAt=0; var _w=0; try{ _w=+(localStorage.getItem('bf.lastWriteAt')||0); }catch(e){} if(_w>_oa && ph.__bfReload){ ph.__bfReload(); } } } return; } var host=null; var els=document.querySelectorAll('p,span,div,h1,h2,h3,h4,td,li,strong,em,b'); for(var i=0;i<els.length;i++){ var e=els[i]; if(e.children.length>0) continue; if((e.textContent||'').trim()==='BF_PIPELINE_FUNNEL'){ host=e; break; } } if(!host) return; if(host.getAttribute('data-bfpipe')==='1') return; host.setAttribute('data-bfpipe','1'); try{ console.log('BF_PIPE rendering funnel'); }catch(e){} bfPipeCSS(); bfPipeRender(host); }catch(e){} }
