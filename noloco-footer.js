...
(function () {
  ...
  function bfPipeRender(R){
    ...
    function bfPipeKPI(){
      ...
      function cohort(T,p,pred){ return recs.filter(function(r){ var d=r.dateListed||r.stageEnteredAt; return (p?inP(d,T):inR(d,T))&&pred(r); }).length; }
      function isAcq(r){ return r.status==='ACQUIRED'; }
      function reachedOffer(r){ return ord(r.status)>=5; }
      var Tc=tf(st.tf.close), ldC=leadsIn(Tc,false), ldP=leadsIn(Tc,true);
      var crC=ldC?Math.round(cohort(Tc,false,isAcq)/ldC*100):0, crP=ldP?Math.round(cohort(Tc,true,isAcq)/ldP*100):0;
      var To=tf(st.tf.offer), loC=leadsIn(To,false), loP=leadsIn(To,true);
      var ocC=loC?Math.round(cohort(To,false,reachedOffer)/loC*100):0, ocP=loP?Math.round(cohort(To,true,reachedOffer)/loP*100):0;
      var kEl=q('kpis');
      if(kEl) kEl.innerHTML=
        ...
        '<div class="bff-kpi bff-kpi-tf"><div class="bff-kpi-top"><div class="bff-kpi-ic"><i class="ti ti-target-arrow"></i></div>'+tfSel('close')+'</div><div class="bff-kpi-v">'+crC+'%</div><div class="bff-kpi-l">Close Rate</div><div class="bff-kpi-sub">'+dhtml(dlt(crC,crP))+'</div></div>'+'<div class="bff-kpi bff-kpi-tf"><div class="bff-kpi-top"><div class="bff-kpi-ic"><i class="ti ti-send"></i></div>'+tfSel('offer')+'</div><div class="bff-kpi-v">'+ocC+'%</div><div class="bff-kpi-l">Offer to Close</div><div class="bff-kpi-sub">'+dhtml(dlt(ocC,ocP))+'</div></div>';
      ...
      var REPL={conversion:'Conversion Rate',vehicles:'Vehicles by Stage',avgdaysstage:'Average Days in Stage',acquisition:'Acquisition Rate',appraisalcomp:'Appraisal Completion Rate',offeracc:'Offer Acceptance Rate',followup:'Follow-up Effectiveness',velocity:'Acquisition Velocity',appraiser:'Appraiser Performance'};
      var RMETA={conversion:'% of leads that convert to the next stage',vehicles:'Total number of vehicles in each stage',avgdaysstage:'Average time vehicles spend in each stage',acquisition:'% of fresh leads that result in an acquisition',appraisalcomp:'% of VINs that are appraised',offeracc:'% of offers that are accepted',followup:'Acquisition rate by number of follow-up touches',velocity:'Average time from lead to acquisition',appraiser:'Per-appraiser appraisal and offer metrics'};
      ...
      function apprReport(){ var by={}; recs.forEach(function(r){ var ap=(r.appraiser||'').trim(); if(!ap) return; if(!by[ap]) by[ap]={asg:0,comp:0,bCM:0,bCV:0,bBoth:0,bNeither:0,cmD:0,cvD:0,bothD:0,days:0,daysN:0}; var a=by[ap]; a.asg++; if(ord(r.status)>=3){ a.comp++; if(r.daysInStage!=null){ a.days+=Number(r.daysInStage||0); a.daysN++; } } var cm=Number(r.carmax),cv=Number(r.carvana),off=Number(r.offer)||Number(r.acv); var hCM=!isNaN(cm)&&cm>0,hCV=!isNaN(cv)&&cv>0; if(hCM){a.cmD++; if(off>cm)a.bCM++;} if(hCV){a.cvD++; if(off>cv)a.bCV++;} if(hCM&&hCV){a.bothD++; if(off>cm&&off>cv)a.bBoth++; if(!(off>cm)&&!(off>cv))a.bNeither++;} }); var ks=Object.keys(by); if(!ks.length) return '<div class="bff-empty">No appraiser data yet.</div>'; function p(n,d){return d?Math.round(n/d*100):0;} var h='<div class="bff-rtblwrap"><table class="bff-rtbl"><thead><tr><th>Appraiser</th><th class="n">Asgnd</th><th class="n">Compl</th><th class="n">Compl %</th><th class="n">Avg Days</th><th class="n">Beat CM</th><th class="n">Beat CV</th><th class="n">Beat Both</th><th class="n">Neither</th></tr></thead><tbody>'; h+=ks.map(function(k){var a=by[k]; return '<tr><td>'+esc2(k)+'</td><td class="n">'+a.asg+'</td><td class="n">'+a.comp+'</td><td class="n">'+p(a.comp,a.asg)+'%</td><td class="n">'+(a.daysN?Math.round(a.days/a.daysN*10)/10:'—')+'</td><td class="n">'+p(a.bCM,a.cmD)+'%</td><td class="n">'+p(a.bCV,a.cvD)+'%</td><td class="n">'+p(a.bBoth,a.bothD)+'%</td><td class="n">'+p(a.bNeither,a.bothD)+'%</td></tr>';}).join(''); return h+'</tbody></table></div>'; }
      function bfPipeReports(){
        var which=st.tf.report||'conversion'; var U=units();
        ...
        function funnel(items,noBars){ var mx=Math.max.apply(null,items.map(function(x){return x.num;}).concat([0.0001])); return '<div class="bff-rfn">'+items.map(function(x,i){ var hh=Math.max(8,Math.round((x.num/mx)*58)); var conn=i>0?'<div class="bff-rfconn"><span class="bff-rfdot"></span></div>':''; return conn+'<div class="bff-rfcol"><div class="bff-rfval">'+x.disp+'</div>'+(noBars?'<div class="bff-rfgap"></div>':'<div class="bff-rfbar" style="height:'+hh+'px"></div>')+'<div class="bff-rfico">'+pSvg(x.label)+'</div><div class="bff-rflbl">'+esc2(x.label)+'</div></div>'; }).join('')+'</div>'; }
        function donut(pct,lbl){ var rr=40,cc=2*Math.PI*rr,off=cc*(1-Math.max(0,Math.min(100,pct))/100); return '<div class="bff-rdc"><svg viewBox="0 0 100 100"><circle cx="50" cy="50" r="'+rr+'" class="bff-rdbg"></circle><circle cx="50" cy="50" r="'+rr+'" class="bff-rdfg" stroke-dasharray="'+cc.toFixed(1)+'" stroke-dashoffset="'+off.toFixed(1)+'" transform="rotate(-90 50 50)"></circle></svg><div class="bff-rdtxt"><div class="bff-rdpct">'+pct+'%</div><div class="bff-rdl">'+lbl+'</div></div></div>'; }
        ...
        else if(which==='vehicles'){ body=funnel(U.map(function(u){return {label:u.label,num:u.count,disp:num(u.count)};})); summ='Total in Pipeline <b>'+num(recs.length)+'</b>'; }
        else if(which==='avgdaysstage'){ body=funnel(U.map(function(u){var a=avgDS(u); return {label:u.label,num:a,disp:a+'<span class="bff-rfu">days</span>'};}),true); var aq=recs.filter(function(r){return r.status==='ACQUIRED'&&r.dateListed&&r.stageEnteredAt;}); var dd=aq.map(function(r){return (Date.parse(r.stageEnteredAt)-Date.parse(r.dateListed))/864e5;}).filter(function(x){return !isNaN(x)&&x>=0;}); var av=dd.length?Math.round(dd.reduce(function(s,x){return s+x;},0)/dd.length*10)/10:0; summ='Average Time to Acquire <b>'+av+' days</b>'; }
        ...
        else if(which==='appraisalcomp'){ var vinR=reached(1),apR=reached(2); body='<div class="bff-rdrow">'+sb('VIN Captured',num(vinR),'ti-barcode')+donut(pctv(apR,vinR),'Completion Rate')+sb('Appraised',num(apR),'ti-clipboard-check')+'</div>'; summ='Total Leads <b>'+num(recs.length)+'</b>'; }
        else if(which==='offeracc'){ var ofR=reached(3),acR=reached(U.length-1); body='<div class="bff-rdrow">'+sb('Offers Sent',num(ofR),'ti-send')+donut(pctv(acR,ofR),'Acceptance Rate')+sb('Offers Accepted',num(acR),'ti-businessplan')+'</div>'; }
        else if(which==='velocity'){ var aq2=recs.filter(function(r){return r.status==='ACQUIRED'&&r.dateListed&&r.stageEnteredAt;}); var dd2=aq2.map(function(r){return (Date.parse(r.stageEnteredAt)-Date.parse(r.dateListed))/864e5;}).filter(function(x){return !isNaN(x)&&x>=0;}); var av2=dd2.length?Math.round(dd2.reduce(function(s,x){return s+x;},0)/dd2.length*10)/10:0; var ss=U.slice(1); var mx2=Math.max.apply(null,ss.map(function(u){return avgDS(u);}).concat([0.0001])); var bars=ss.map(function(u){var a=avgDS(u); var hh=Math.max(8,Math.round((a/mx2)*54)); return '<div class="bff-rfcol"><div class="bff-rfval">'+a+'<span class="bff-rfu">days</span></div><div class="bff-rfbar" style="height:'+hh+'px"></div><div class="bff-rflbl">'+esc2(u.label)+'</div></div>';}).join(''); body='<div class="bff-rvrow"><div class="bff-rvbox"><div class="bff-rvboxl">Average Time<br>Lead → Acquisition</div><div class="bff-rvbig">'+av2+' <span>Days</span></div><i class="ti ti-clock bff-rvclock"></i></div><div class="bff-rfn bff-rvbars">'+bars+'</div></div>'; }
        else if(which==='followup'){ var bk={}; recs.forEach(function(r){ var t=Math.min(4,Number(r.followups||0)); if(!bk[t])bk[t]={tot:0,acq:0}; bk[t].tot++; if(r.status==='ACQUIRED')bk[t].acq++; }); var its=[0,1,2,3,4].map(function(t){var b=bk[t]||{tot:0,acq:0}; return {l:(t===4?'4+ Touches':(t+' Touch'+(t===1?'':'es'))),v:pctv(b.acq,b.tot)};}); var mxf=Math.max.apply(null,its.map(function(x){return x.v;}).concat([1])); body='<div class="bff-rfbig">'+its.map(function(x){var hh=Math.max(8,Math.round((x.v/mxf)*120)); return '<div class="bff-rfbcol"><div class="bff-rfbv">'+x.v+'%</div><div class="bff-rfbbar" style="height:'+hh+'px"></div><div class="bff-rfblbl">'+esc2(x.l)+'</div></div>';}).join('')+'</div>'; }
        else if(which==='appraiser'){ body=apprReport(); }
        ...
        var rEl=q('report'); if(rEl) rEl.innerHTML='<div class="bff-rhead"><div class="bff-rhl"><div class="bff-rtitle">'+REPL[which]+'</div><div class="bff-rdesc">'+RMETA[which]+'</div></div>'+reportSel()+'</div><div class="bff-rbody">'+body+'</div>'+foot;
      }
      bfPipeReports();
    ...
    function load(dealId){ st.dealId=(dealId!=null?dealId:null); var tk=''; try{ tk=bfGetApiToken(); }catch(e){} if(!tk){ waitToken(); return; } var bo=st.dealId!=null?{dealershipId:st.dealId}:{}; fetch(ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json','X-BF-Token':tk},body:JSON.stringify(bo)}).then(function(r){return r.json();}).then(function(d){ st.data=d||{stages:[]}; if(d&&d.needsAuth){ var s2=q('scope'); if(s2) s2.textContent='(access not recognized — open any record once to refresh)'; return; } decorate(st.data); updateScope(d); buildFilter(); refresh(); }).catch(function(){ q('funnel').innerHTML='<div class="bff-empty">Could not load the pipeline. Refresh to retry.</div>'; }); }
    ...
    (function(){ var fb=q('fbtn'); if(fb) fb.addEventListener('click',function(){ var fp=q('fpanel'); fp.style.display=(fp.style.display==='none'?'block':'none'); }); })();
