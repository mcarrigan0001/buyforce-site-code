...
(function () {
  ...
  var bfPipeBd=null,bfPipeAL=null,bfPipeAR=null; var BF_PIPE_EL='vewCcJTaXm4WOtOcuaCNz1a7';
  ...
  function bfPipeDrawer(){
    ...
    var rv=document.querySelector('[data-testid="record-view"]'); if(!rv){ if(bfPipeBd)bfPipeBd.style.display='none'; return; }
  ...
  function bfPipeRender(R){
    ...
    function openOverlay(uuid){ try{ var _na=document.querySelector('[data-testid="collection-record"][href*="'+uuid+'"]'); if(_na){ window.__bfPipeDrawerOpenedAt=Date.now(); try{ sessionStorage.setItem('bf.pipeorder', JSON.stringify(navList())); }catch(_o){} _na.click(); return; } }catch(_e){} ensureOverlay(); st.ovlOpenedAt=Date.now(); ovlList=navList(); ovlIdx=ovlList.indexOf(uuid); if(ovlIdx<0){ ovlList=[uuid]; ovlIdx=0; } ovl.style.display='block'; ovlShow(); }
