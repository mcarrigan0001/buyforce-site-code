...
(function () {
  ...
  function bfPipeRender(R){ try{ R.style.visibility='visible'; if(window.__bfStopPrehide) window.__bfStopPrehide(); }catch(_e){}
    ...
    function openOverlay(uuid){ try{ sessionStorage.setItem('bf.pipeorder', JSON.stringify(navList())); }catch(_o){} try{ sessionStorage.setItem('bf.navts', String(Date.now())); }catch(_t){} location.assign('/command/preview/'+uuid+'/overview'); }
