...
(function () {
  ...
  function bfPipeRender(R){ try{ R.style.visibility='visible'; if(window.__bfStopPrehide) window.__bfStopPrehide(); }catch(_e){}
    ...
    function openOverlay(uuid){ try{ sessionStorage.setItem('bf.pipeorder', JSON.stringify(navList())); }catch(_o){} ensureOverlay(); st.ovlOpenedAt=Date.now(); ovlList=navList(); ovlIdx=ovlList.indexOf(uuid); if(ovlIdx<0){ ovlList=[uuid]; ovlIdx=0; } ovl.style.display='block'; ovlShow(); }
