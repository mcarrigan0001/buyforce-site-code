...
(function () {
  ...
  function bfPipeRender(R){
    ...
    function bfPipeKPI(){
      ...
      var rsP=repRecs('perf'); var Tp=tf(st.tf.perf); var byRep={}; rsP.forEach(function(r){ if(r.status!=='ACQUIRED'||!inR(r.stageEnteredAt,Tp)) return; var rp=(r.rep||'').trim(); if(!rp) return; byRep[rp]=(byRep[rp]||0)+1; });
      ...
      function bfPipeReports(){
        ...
        else if(which==='appraisalcomp'){ var vinR=reached(1),apR=reached(2); body='<div class="bff-rdrow">'+sb('VIN Captured',num(vinR),'ti-barcode')+donut(pctv(apR,vinR),'Completion Rate')+sb('Appraised',num(apR),'ti-clipboard-check')+'</div>'; summ='Total Leads <b>'+num(rrecs.length)+'</b>'; }
        ...
        var rEl=q('report'); if(rEl) rEl.innerHTML='<div class="bff-rhead"><div class="bff-rhl"><div class="bff-rtitle">'+REPL[which]+'</div><div class="bff-rdesc">'+RMETA[which]+'</div></div><div class="bff-rhc">'+reportSel()+ddRep('report')+ddTF('reporttf')+'</div></div><div class="bff-rbody">'+body+'</div>'+foot;
      }
      bfPipeReports();
      var mrEl=q('masterrep'); if(mrEl) mrEl.innerHTML='<span class="bff-mrl">Rep:</span>'+ddRep('master');
