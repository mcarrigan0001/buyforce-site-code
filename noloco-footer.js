...
(function () {
  ...
  function bfPipeRender(R){
    ...
    R.innerHTML='<div id="bff-wrap"><div class="bff-head"><span class="bff-title bff-glow">Command Center</span><span class="bff-scope" data-r="scope"></span><div class="bff-rt"><span class="bff-masterrep" data-r="masterrep"></span><div class="bff-sectoggles"><button class="bff-sectg on" data-sec="kpis" type="button"><i class="ti ti-gauge"></i>KPIs</button><button class="bff-sectg on" data-sec="extras" type="button" title="Leaderboard & Reports"><i class="ti ti-chart-histogram"></i></button></div><div class="bff-tabs"><button class="bff-tab on" data-view="macro">Milestones</button><button class="bff-tab" data-view="stages">All stages</button></div></div></div><div class="bff-kpis" data-r="kpis"></div><div class="bff-extras"><div class="bff-actpanel" data-r="actpanel"></div><div class="bff-perfpanel" data-r="perf"></div><div class="bff-reportpanel" data-r="report"></div></div><div class="bff-funnel" data-r="funnel"><div class="bff-load">Loading pipeline…</div></div><div class="bff-caret" data-r="caret" style="display:none;" title="Show all opportunities"><i class="ti ti-chevron-down" aria-hidden="true"></i></div></div><div class="bff-listcard" data-r="lc"></div>'; (function(){ [].forEach.call(R.querySelectorAll('.bff-sectg'),function(b){ b.addEventListener('click',function(){ var sec=b.getAttribute('data-sec'); var tg=(sec==='kpis')?q('kpis'):R.querySelector('.bff-extras'); var on=b.classList.toggle('on'); if(tg) tg.style.display=on?'':'none'; }); }); })();
    ...
    function bfPipeKPI(){
      ...
      function ddTF(w){ var k=effTF(w); var lab=(k!=='mtd')?('<span class="bff-ddl">'+TFL[k]+'</span>'):''; var m=Object.keys(TFL).map(function(x){return '<div class="bff-ddo'+(x===k?' on':'')+'" data-dd="'+w+'" data-kind="tf" data-v="'+x+'">'+TFL[x]+'</div>';}).join(''); return '<div class="bff-dd bff-ddt"><button type="button" class="bff-ddb" data-ddtoggle="1" title="Timeframe"><i class="ti ti-clock-hour-4"></i>'+lab+'<i class="ti ti-chevron-down bff-ddcar"></i></button><div class="bff-ddm">'+m+'</div></div>'; }
      function ddRep(w){ var rp=effRep(w); var face=rp?('<span class="bff-ddav">'+esc2(initials(rp))+'</span>'):'<i class="ti ti-user"></i>'; var opts='<div class="bff-ddo'+(rp===''?' on':'')+'" data-dd="'+w+'" data-kind="rep" data-v="">All Reps</div>'+REPS.map(function(rn){return '<div class="bff-ddo'+(rp===rn?' on':'')+'" data-dd="'+w+'" data-kind="rep" data-v="'+esc2(rn)+'"><span class="bff-ddav sm">'+esc2(initials(rn))+'</span>'+esc2(rn)+'</div>';}).join(''); return '<div class="bff-dd bff-ddr"><button type="button" class="bff-ddb" data-ddtoggle="1" title="Rep">'+face+'<i class="ti ti-chevron-down bff-ddcar"></i></button><div class="bff-ddm bff-ddmr">'+opts+'</div></div>'; }
      ...
      var rso=repRecs('offer'); var To=tf(effTF('offer')), loC=leadsIn(rso,To,false), loP=leadsIn(rso,To,true);
      ...
      var closeAcq=rsc.filter(function(r){return r.status==='ACQUIRED';}).length, closeLeads=rsc.length; var offerAcq=rso.filter(function(r){return r.status==='ACQUIRED';}).length, offers=rso.filter(function(r){return ord(r.status)>=4;}).length;
      var kEl=q('kpis');
      ...
      if(kEl) kEl.innerHTML=
        '<div class="bff-kpi bff-kpi-tf"><span class="bff-kpi-rep">'+ddRep('active')+'</span><div class="bff-kpi-body"><div class="bff-kpi-ic" data-kpiic="active"><i class="ti ti-flame"></i></div><div class="bff-kpi-main"><div class="bff-kpi-v">'+actv.length.toLocaleString('en-US')+'</div><div class="bff-kpi-l">Active Deals</div></div></div></div>'+
        '<div class="bff-kpi bff-kpi-tf"><span class="bff-kpi-rep">'+ddRep('acq')+'</span><span class="bff-kpi-time">'+ddTF('acq')+'</span><div class="bff-kpi-body"><div class="bff-kpi-ic" data-kpiic="acq"><i class="ti ti-checks"></i></div><div class="bff-kpi-main">'+acqBody+'</div></div></div>'+
        '<div class="bff-kpi bff-kpi-tf"><span class="bff-kpi-rep">'+ddRep('close')+'</span><span class="bff-kpi-time">'+ddTF('close')+'</span><div class="bff-kpi-body"><div class="bff-kpi-ic" data-kpiic="close"><i class="ti ti-target-arrow"></i></div><div class="bff-kpi-main"><div class="bff-kpi-v">'+crC+'%</div><div class="bff-kpi-l">Close Rate</div><div class="bff-kpi-sub">'+dhtml(dlt(crC,crP))+'</div></div></div>'+(st.kpiInfo&&st.kpiInfo.close?'<div class="bff-kpi-info">'+closeAcq+' acquired out of '+closeLeads+' leads</div>':'')+'</div>'+
        '<div class="bff-kpi bff-kpi-tf"><span class="bff-kpi-rep">'+ddRep('offer')+'</span><span class="bff-kpi-time">'+ddTF('offer')+'</span><div class="bff-kpi-body"><div class="bff-kpi-ic" data-kpiic="offer"><i class="ti ti-send"></i></div><div class="bff-kpi-main"><div class="bff-kpi-v">'+ocC+'%</div><div class="bff-kpi-l">Offer to Close</div><div class="bff-kpi-sub">'+dhtml(dlt(ocC,ocP))+'</div></div></div>'+(st.kpiInfo&&st.kpiInfo.offer?'<div class="bff-kpi-info">'+offerAcq+' acquired out of '+offers+' offers</div>':'')+'</div>';
      ...
      var aEl=q('actpanel'); if(aEl) aEl.innerHTML='<div class="bff-ph"><span>Activity</span><span class="bff-phc">'+ddRep('activity')+ddTF('activity')+'</span></div>'+actDefs.map(function(a){ var c=actCount(a.ss,Tac,false), pv=actCount(a.ss,Tac,true); return '<div class="bff-actrow"><span class="bff-actn">'+c+'</span><span class="bff-actl">'+a.l+'</span>'+dhtml(dlt(c,pv))+'</div>'; }).join('');
      var rsP=repRecs('perf'); var Tp=tf(effTF('perf')); var byRep={}; rsP.forEach(function(r){ if(r.status!=='ACQUIRED'||!inR(r.stageEnteredAt,Tp)) return; var rp=(r.rep||'').trim(); if(!rp) return; byRep[rp]=(byRep[rp]||0)+1; });
      var perf=Object.keys(byRep).map(function(k){return {name:k,n:byRep[k]};}).sort(function(a,b){return b.n-a.n;}).slice(0,5);
      var pEl=q('perf'); if(pEl) pEl.innerHTML='<div class="bff-ph"><span>Top Performers</span><span class="bff-phc">'+ddRep('perf')+ddTF('perf')+'</span></div>'+(perf.length?perf.map(function(p,i){ var ini=initials(p.name); return '<div class="bff-perfrow"><span class="bff-perfrank">'+(i+1)+'</span><span class="bff-perfav">'+esc2(ini)+'</span><span class="bff-perfn">'+esc2(p.name)+'</span><span class="bff-perfc">'+p.n+'</span></div>'; }).join(''):'<div class="bff-empty">No acquisitions in this period.</div>');
      ...
      function bfPipeReports(){
        ...
        var rEl=q('report'); if(rEl) rEl.innerHTML='<div class="bff-rhead"><div class="bff-rhl"><div class="bff-rtitle">'+REPL[which]+'</div><div class="bff-rdesc">'+RMETA[which]+'</div></div><div class="bff-rhc"><div class="bff-rhc-top">'+reportSel()+'</div><div class="bff-rhc-bot">'+ddRep('report')+ddTF('reporttf')+'</div></div></div><div class="bff-rbody">'+body+'</div>'+foot;
      }
      bfPipeReports();
      var mrEl=q('masterrep'); if(mrEl) mrEl.innerHTML=ddRep('master')+ddTF('master');
