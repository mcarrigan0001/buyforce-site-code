...
(function () {
  ...
  function bfPipeRender(R){
    ...
    function rawUnits(){ var s=(st.data&&st.data.stages)||[]; if(st.view==='stages') return s; var by={}; s.forEach(function(x){by[x.status]=x;}); return MACRO.map(function(m){ var recs=[]; m.statuses.forEach(function(cd){ var z=by[cd]; if(z) recs=recs.concat(z.records||[]); }); return {status:m.label,label:m.label,records:recs}; }); }
