...
(function () {
  ...
  function bfPipeRender(R){
    ...
    function openOverlay(uuid){ try{ var _na=document.querySelector('[data-testid="collection-record"][href*="'+uuid+'"]'); if(_na){ window.__bfPipeDrawerOpenedAt=Date.now(); _na.click(); return; } }catch(_e){} ensureOverlay(); st.ovlOpenedAt=Date.now(); ovlList=navList(); ovlIdx=ovlList.indexOf(uuid); if(ovlIdx<0){ ovlList=[uuid]; ovlIdx=0; } ovl.style.display='block'; ovlShow(); }
