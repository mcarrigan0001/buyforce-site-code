...
(function () {
  ...
  function bfWsHtml(stg, F){
    ...
    var _pick=bfGet(F,['Pickup Address','pickupAddress'])||'';
  ...
  function bfWsAction(ws, uuid, act){
    ...
    if(act==='book'){ var _di=ws.querySelector('.bf-ws-dateinput'); var _ti=ws.querySelector('.bf-ws-timeinput'); var day=(_di&&_di.value)||''; var tm=(_ti&&_ti.value)||''; if(!day){ var dEl=ws.querySelector('.bf-ws-type.sel[data-day]'); day=dEl?dEl.getAttribute('data-day'):''; } if(!tm){ var tEl=ws.querySelector('.bf-ws-type.sel[data-time]'); tm=tEl?tEl.getAttribute('data-time'):''; } if(!day||!tm){ bfToast('Pick a date and time'); return; } var dObj=new Date(day+'T'+tm+':00'); var iso=isNaN(dObj.getTime())?'':dObj.toISOString(); var human=isNaN(dObj.getTime())?(day+' '+tm):(dObj.toLocaleDateString(undefined,{weekday:'short',month:'short',day:'numeric'})+' at '+dObj.toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'})); var deal=ws._bfDeal||{}; var locPill=ws.querySelector('.bf-ws-type.sel[data-loc]'); var locKind=locPill?locPill.getAttribute('data-loc'):'dealership'; var locStr=deal.address||''; var payload={uuid:uuid, status:'Scheduled', appointmentDateTime:iso}; if(locKind==='pickup'){ var _pi=ws.querySelector('.bf-ws-pickupinput'); var _pa=(_pi&&_pi.value||'').trim(); locStr=_pa; if(_pa) payload.pickupAddress=_pa; } bfPost(payload); var locTxt=locStr?(' \u2014 '+(locKind==='pickup'?'pickup at ':'at ')+locStr):''; bfPostEvent({uuid:uuid, type:'appt_booked', actor:actor, text:'Appointment booked for '+human+locTxt}); bfWsPrepend(ws,{type:'appt_booked',actor:actor,text:'Appointment booked for '+human+locTxt,createdAt:nowI}); bfToast('Booked \u00b7 moved to Scheduled'); var conf=ws.querySelector('.bf-ws-confirm'); if(conf){ var ctext=bfWsConfirmText(deal, human, locStr); conf.innerHTML='<div class="bf-ws-sublbl" style="margin-top:16px;">Confirmation text (send the evening before)</div><div class="bf-ws-track"><div class="bf-ws-trh"><span class="bf-ws-trn">Day-before reminder</span><button class="bf-ws-copy"><i class="ti ti-copy" aria-hidden="true"></i>Copy</button></div><div class="bf-ws-track-txt">'+esc(ctext)+'</div></div>'; } return; }
