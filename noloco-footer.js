...
(function () {
  ...
  function bfPipeRender(R){
    ...
    function bfPipeKPI(){
      ...
      var perf=Object.keys(byRep).map(function(k){return byRep[k];}).sort(function(a,b){return (b.acq-a.acq)||(b.total-a.total);}).slice(0,3);
      var pEl=q('perf'); if(pEl) pEl.innerHTML='<div class="bff-ph"><span>Top Performers</span><span class="bff-phc">'+ddRep('perf')+ddTF('perf')+'</span></div>'+(perf.length?('<div class="bff-tplist">'+perf.map(function(p,i){ var cl=p.total?Math.round(p.acq/p.total*100):0; return '<div class="bff-tpitem"><div class="bff-tprank">#'+(i+1)+'</div><div class="bff-tpav">'+esc2(initials(p.name))+'</div><div class="bff-tpinfo"><div class="bff-tpname">'+esc2(p.name)+'</div><div class="bff-tprole">'+p.total+' Opportunities</div></div><div class="bff-tpmet"><div class="bff-tpval">'+p.acq+'</div><div class="bff-tplbl">'+cl+'% Close</div></div></div>'; }).join('')+'</div>'):'<div class="bff-empty">No reps assigned.</div>');
